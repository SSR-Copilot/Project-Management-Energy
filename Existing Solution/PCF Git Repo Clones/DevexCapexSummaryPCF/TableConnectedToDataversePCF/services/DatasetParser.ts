/**
 * ============================================================================
 * DatasetParser — flat-dataset → tree converter for the "DevexCapexSummaryPCF"
 * Power Apps PCF control (Canvas App DEVEX/CAPEX cost summary screen).
 * ============================================================================
 *
 * WHY THIS FILE EXISTS
 * The Canvas App builds a FLAT collection `colFinatCostDataOptimized` (the typo
 * in the name is intentional/legacy — do not "fix" it on the Canvas side) bound
 * to the `DevexCapexCostSummary` dataset, plus `colStandardContractOptionsForPCF`
 * bound to the `StandardContractOptions` dataset. This class is THE single
 * integration point that converts those flat PCF dataset rows into the nested
 * AccountData → SubaccountData → ContractData model (see ../models/GridModels)
 * consumed by GridRenderer. Canvas owns ALL Dataverse reads/writes and business
 * rules; this parser only reshapes what Canvas sends — it must never gain
 * Dataverse logic of its own.
 *
 * HOW ROWS ARE LINKED
 * Every row carries `RowId`, `ParentId` and a `Type` discriminator with values
 * "account" | "subaccount" | "contract" (matched case-insensitively). Contracts
 * attach to subaccounts, and subaccounts to accounts, by ParentId → RowId
 * equality AFTER normalizeId() trims/lowercases both sides (Dataverse GUID
 * casing differs between APIs/sources). Rows whose ParentId matches nothing are
 * silently dropped — there is no orphan bucket.
 *
 * MONEY SEMANTICS — DO NOT CHANGE WITHOUT RE-TESTING IN THE CANVAS APP
 * `M1`..`M12` hold monthly costs for the SELECTED YEAR ONLY. `TotalCost`,
 * `PlannedCost` and `ActualCost` are ALL-YEARS totals pre-computed in Canvas.
 * The parser deliberately reads those total columns instead of summing M1..M12:
 * summing the visible months was a past bug that displayed selected-year-only
 * numbers in the "all years" total columns.
 *
 * WHY SO MANY FIELD-NAME ALIASES
 * Canvas collection column names, Dataverse logical names (vsb_*) and display
 * names all differ in casing/punctuation, and choice columns arrive in
 * inconsistent shapes (boolean, 0/1, string label, or {Value/Label} objects).
 * Field names are therefore matched case-insensitively after stripping every
 * non-alphanumeric character (see normalizeFieldName), and values go through
 * tolerant parsers (parseBoolean/parseText/parseNumber/normalizeId). Some flags
 * can additionally be smuggled inside SubLabel as `||KEY=value||` metadata
 * tokens (e.g. ||STD=1|| marks a standard contract, ||PAID=1,2,3|| marks paid
 * months) for cases where the Canvas collection cannot carry an extra column.
 *
 * THINGS THAT MUST NOT CHANGE WITHOUT END-TO-END TESTING AGAINST THE APP:
 *   - canonical field names and every alias list (RowId/ParentId/Type/Name/
 *     SubLabel/M1..M12/M{n}Paid/TotalCost/PlannedCost/ActualCost/CostPaidBy/
 *     DistributionType/comment tooltip aliases/standard-contract aliases)
 *   - the Type discriminator values "account"/"subaccount"/"contract"
 *   - RowId/ParentId matching and normalizeId() lowercasing
 *   - the all-years total semantics described above
 *   - the SubLabel ||KEY=value|| token format
 *
...
import { AccountData, SubaccountData, ContractData, ClusterSpan, ClusterTimeline, StandardContractOption } from "../models/GridModels";
...
export class DatasetParser {
     ...
     */
    public static parseDatasetToTree(dataset: ComponentFramework.PropertyTypes.DataSet): AccountData[] {
        ...
        columns.forEach(col => {
            ...
            if (["rowid", "id", "vsbcapexaccountlistid", "capexaccountlist", "capexprojectcontract"].includes(normalizedName)) rowIdField = col.name;
            else if (["parentid", "vsbparentaccountid", "parentaccount"].includes(normalizedName)) parentIdField = col.name;
            else if (normalizedName === "type") typeField = col.name;
            else if (normalizedName === "number" || normalizedName === "vsbnumber") numberField = col.name;
            else if (["name", "vsbname", "vsbcostdescription", "costdescription"].includes(normalizedName)) nameField = col.name;
            else if (normalizedName === "sublabel") subLabelField = col.name;
            else if (["costpaidby", "costpaidbyfilter", "vsbcostpaidby", "costtype", "vsbcosttype"].includes(normalizedName)) costPaidByField = col.name;
            else if (["totalcost", "totalcosts", "vsbtotalcost", "totalcostamount"].includes(normalizedName)) totalCostField = col.name;
            else if (["plannedcost", "totalplannedcost", "totalplannedcosts"].includes(normalizedName)) plannedCostField = col.name;
            else if (["actualcost", "totalactualcost", "totalactualcosts"].includes(normalizedName)) actualCostField = col.name;
            else if (normalizedName === "isaccount") isAccountField = col.name;
            else if (normalizedName === "iscontract") isContractField = col.name;
            ...
            // detection for the data shape that uses it.
                ...
                "vsb_isstandardcost"
            ].includes(normalizedName)) isStandardContractField = col.name;
            else if (["standardcontractoptions", "standardcontracts", "availablestandardcontracts"].includes(normalizedName)) standardContractOptionsField = col.name;
            ...
            ].includes(normalizedName)) distributionTypeField = col.name;
            ...
            // renamed several times in the Canvas collection over the project history.
                ...
                "latestcomment",
                "commentsummary"
            ].includes(normalizedName)) commentTooltipField = col.name;
            ...
            for (let i = 1; i <= 12; i++) {
                if (normalizedName === `m${i}` || normalizedName === `month${i}`) {
                    ...
                    normalizedName === `m${i}hascomments`
                    ...
                    normalizedName === `m${i}commenttooltip`
        ...
        const subaccountsMap = new Map<string, SubaccountData>();
        ...
        dataset.sortedRecordIds.forEach(id => {
            ...
            const costPaidBy = this.parseCostPaidBy(
                this.firstAvailableValue(record, [
                    costPaidByField,
                    "CostPaidBy",
                    "costPaidBy",
                    "CostPaidByText",
            ...
            const distributionType = this.parseText(
                this.firstAvailableValue(record, [
            ...
            const months = mFields.map((fieldName, index) => this.parseNumber(
                this.firstAvailableValue(record, [
                    fieldName,
                    `M${index + 1}`,
                    `m${index + 1}`,
            ...
            if (finalType === "account") {
                ...
                subaccountsMap.set(rowId, {
                    ...
                    standardContractOptions: this.parseStandardContractOptions(
                        this.firstAvailableValue(record, [
     ...
     */
    public static parseStandardContractOptionsDataset(dataset: ComponentFramework.PropertyTypes.DataSet): Map<string, StandardContractOption[]> {
        const optionsBySubaccount = new Map<string, StandardContractOption[]>();
        ...
        dataset.columns.forEach(col => {
            ...
            const normalizedName = this.normalizeFieldName(col.name);
            if (
                normalizedName === "standardcontractid"
     ...
     */
    private static parseStandardContractOptions(value: unknown): StandardContractOption[] {
     ...
     */
    private static safeGetValue(record: ComponentFramework.PropertyHelper.DataSetApi.EntityRecord, fieldName: string): unknown {
        if (!fieldName) {
            return null;
        }
     ...
     */
    private static safeGetFormattedValue(record: ComponentFramework.PropertyHelper.DataSetApi.EntityRecord, fieldName: string): string {
        if (!fieldName) {
            return "";
        }
     ...
     */
    private static firstAvailableValue(record: ComponentFramework.PropertyHelper.DataSetApi.EntityRecord, fieldNames: string[]): unknown {
     ...
     */
    private static extractMetaValue(source: string, key: string): string {
        if (!source || !key) {
            return "";
        }
     ...
     */
    private static removeMetaTokens(source: string): string {
        return (source || "")
            .replace(/\|\|\s*[^|=]+?\s*=\s*[^|]*?\s*\|\|/g, "")
            .replace(/\s+/g, " ")
            .trim();
     ...
     */
    private static parsePaidMonthsFromSubLabel(subLabel: string): boolean[] {
     ...
     */
    private static parseDistributionTypeFromText(value: string): string {
     ...
     */
    private static parseCommentTooltip(value: unknown): string {
     ...
     */
    private static parseCostPaidBy(value: unknown, subLabel: string): string {
        const rawValue = (value ?? "").toString().trim();
     ...
     */
    private static normalizeDate(value: Date | string | null | undefined): Date | null {
        if (!value) {
            return null;
        }
     ...
     */
    public static calculateClusterTimeline(context: ComponentFramework.Context<IInputs>): ClusterTimeline {
        ...
        const clusterStarts = [
            ...
            .filter((entry): entry is { clusterIndex: number; startMonth: number } => entry !== null && entry.startMonth < 12)
     ...
     */
    public static calculateClusterSpans(context: ComponentFramework.Context<IInputs>): ClusterSpan[] {
