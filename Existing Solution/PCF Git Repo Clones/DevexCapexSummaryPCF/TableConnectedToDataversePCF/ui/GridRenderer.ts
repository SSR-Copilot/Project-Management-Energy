/**
 * GridRenderer — builds and updates the DOM for the "DevexCapexSummaryPCF" control
 * (namespace DEV.Controls), the hierarchical budget grid on the Canvas App
 * DEVEX/CAPEX cost screen.
 *
 * DATA FLOW
 * - Canvas builds the flat collection `colFinatCostDataOptimized` ("Finat" typo is
 *   intentional/legacy — do not "fix" it) and binds it to dataset `DevexCapexCostSummary`.
 *   DatasetParser turns that flat list (linked via RowId/ParentId, `Type` =
 *   account / subaccount / contract) into the Account -> Subaccount -> Contract tree
 *   rendered here.
 * - A second collection `colStandardContractOptionsForPCF` binds to dataset
 *   `StandardContractOptions`; its per-subaccount options feed the "Add Standard
 *   Contract" submenu (see applyStandardContractOptions / showAddDropdown).
 * - M1..M12 hold monthly costs for the SELECTED year only (`StartYear` input), while
 *   TotalCost/PlannedCost/ActualCost are ALL-YEARS totals computed in Canvas. The PCF
 *   must NEVER derive all-year totals by summing M1..M12 — that was a past bug.
 *
 * RESPONSIBILITY SPLIT (important design note)
 * - This class is rendering + interaction ONLY. Canvas owns ALL Dataverse operations
 *   and business rules. Do NOT call Dataverse / WebApi from here. User actions
 *   (Add, AddStandardContract, Edit, Delete, Comment, SetCostPaidUnpaid) are funneled
 *   through `_onActionTriggered`, which index.ts converts into output trigger booleans
 *   plus SelectedRecordId / SelectedStandardContractId / SelectedCostContractId /
 *   SelectedCostYear / SelectedCostMonth / SelectedCostPaid for Canvas to act on.
 * - index.ts orchestrates lifecycle/re-renders (see the PCF_RE_RENDER custom event in
 *   toggleCollapse); ActionDropdown renders the contract row three-dot menu.
 * - If the main dataset is empty, the grid shows its real empty/loading state
 *   (the sample-data fallback was removed on request).
 *
 * FRAGILE AREAS — read the comments before touching:
 * - Tree connector class assignment in createSubaccountRow / createContractRow /
 *   createAddRow: several past regressions (vertical line intruding into the previous
 *   parent row, connector continuing into the Add row, half-filled number column).
 * - renderColGroup width math: fixed columns + 12 equal month columns; the table is
 *   sized explicitly and horizontally scrolls instead of squeezing.
 * - Sticky header + sticky grand total: syncGrandTotalSpacer / applyViewportHeight
 *   keep the grand-total row pinned to the bottom of the viewport.
 *
 * NOTE: all temporary console.log diagnostics were removed on request (2026-06-12)
 * so no dataset contents leak into the browser console in production.
 */
import { IInputs } from "../generated/ManifestTypes";
import { AccountData, SubaccountData, ContractData } from "../models/GridModels";
import { DatasetParser } from "../services/DatasetParser";
import { ActionDropdown } from "./ActionDropdown";

// Action names must stay in sync with the switch in index.ts that maps each one to its
// output trigger boolean — renaming a value here silently breaks the Canvas handshake.
type GridAction = "Edit" | "Comment" | "Delete" | "Add" | "AddStandardContract" | "SetCostPaidUnpaid";

// Payload for "SetCostPaidUnpaid": index.ts copies these into SelectedCostYear /
// SelectedCostMonth (1-based) / SelectedCostPaid so Canvas knows which MxPaid flag to flip.
...
export class GridRenderer {
    ...
    private static readonly NUMBER_COLUMN_MIN_WIDTH = 100;
    // Deliberately raised to 300 so contract names get room before the ellipsis kicks in.
    private static readonly ACCOUNT_NAME_COLUMN_MIN_WIDTH = 300;
    private static readonly TOTAL_COST_COLUMN_MIN_WIDTH = 100;
    // Planned/Actual are slightly wider because their header captions are longer.
    private static readonly PLANNED_COST_COLUMN_MIN_WIDTH = 120;
    private static readonly ACTUAL_COST_COLUMN_MIN_WIDTH = 120;
    // 65px is the floor at which a 6-digit monthly cost still fits without wrapping.
    private static readonly MONTH_COLUMN_MIN_WIDTH = 65;
    // Fixed (not minimum) — the three-dot button never needs to flex.
    private static readonly ACTION_COLUMN_WIDTH = 40;
    ...
    private _lastShowPlanned = false;
    ...
    private _addDropdownElement: HTMLDivElement | null = null;
    ...
    private _activeAddMenuButton: HTMLElement | null = null;
    private _boundAddDropdownWindowClick: (e: MouseEvent) => void;
    private _monthDropdownElement: HTMLDivElement | null = null;
    // Month cell currently highlighted while its dropdown is open, so the "active"
    // class can be removed on close.
    private _activeMonthCell: HTMLTableCellElement | null = null;
    private _commentTooltipElement: HTMLDivElement | null = null;
    private _boundMonthDropdownWindowClick: (e: MouseEvent) => void;
    private _onActionTriggered: (action: GridAction, recordId: string, standardContractId?: string, costPaidPayload?: CostPaidActionPayload) => void;
    ...
    constructor(
        ...
        onActionTriggered: (action: GridAction, recordId: string, standardContractId?: string, costPaidPayload?: CostPaidActionPayload) => void
    ) {
     ...
     * not change totals.
     */
    public render(context: ComponentFramework.Context<IInputs>): void {
     ...
     */
    private fitColumnsToScrollViewport(showPlanned: boolean): void {
     ...
     */
    private applyViewportHeight(availableHeight: number): void {
     ...
     */
    private syncGrandTotalSpacer(totalColSpan: number): void {
        const existingSpacer = this._tbody.querySelector(".pcf-row-footer-spacer");
        if (existingSpacer) {
            existingSpacer.remove();
        }

        const grandRow = this._tbody.querySelector(".pcf-row-grand") as HTMLTableRowElement | null;
        if (!grandRow) {
            return;
        }
        ...
        const renderedBodyHeight = bodyRows.reduce((total, row) => total + (row as HTMLElement).offsetHeight, 0);
     ...
     */
    private applyStandardContractOptions(accounts: AccountData[], optionsBySubaccount: Map<string, { id: string; name: string }[]>): void {
     ...
     */
    private applyCostPaidByFilter(accounts: AccountData[], filterValue: string): AccountData[] {
     ...
     */
    private contractMatchesCostPaidBy(contract: ContractData, expectedCostPaidBy: string): boolean {
     ...
     */
    private removeEmptyAccounts(accounts: AccountData[]): AccountData[] {
        return accounts
            .map(account => {
    ...
    private hasAnyCost(row: { totalCost: number; plannedCost: number; actualCost: number; months: number[] }): boolean {
     ...
     */
    private recalculateAccount(account: AccountData): AccountData {
    ...
    private recalculateSubaccount(subaccount: SubaccountData): SubaccountData {
    ...
    private sumMonths(rows: Array<{ months: number[] }>): number[] {
     ...
     */
    private renderColGroup(showPlanned: boolean, availableWidth: number): void {
        ...
        if (showPlanned) {
            widths.push(
                GridRenderer.PLANNED_COST_COLUMN_MIN_WIDTH,
                GridRenderer.ACTUAL_COST_COLUMN_MIN_WIDTH
            );
        }
     ...
     */
    private renderHeaders(context: ComponentFramework.Context<IInputs>, showPlanned: boolean, precedingColSpan: number): void {
        ...
        const monthClusterStarts = new Set<number>();
     ...
     */
    private createAccountRow(acc: AccountData, isCollapsed: boolean, showPlanned: boolean): HTMLTableRowElement {
     ...
     */
    private createSubaccountRow(
        sub: SubaccountData,
        isCollapsed: boolean,
        showPlanned: boolean,
        isLastSubaccount: boolean,
        connectsFromAccountToggle: boolean
    ): HTMLTableRowElement {
        const tr = document.createElement("tr");
     ...
     */
    private createContractRow(
        contract: ContractData,
        showPlanned: boolean,
        isLastSubaccount: boolean,
        connectsFromSubaccountToggle: boolean,
    ...
    private createGroupGapRow(colSpan: number): HTMLTableRowElement {
        const tr = document.createElement("tr");
     ...
     */
    private isContractFullyPaid(contract: ContractData): boolean {
        return contract.totalCost > 0 && Math.abs(contract.totalCost - contract.actualCost) < 0.005;
    }

    private appendNumericCell(tr: HTMLTableRowElement, val: number, className = ""): void {
        const td = document.createElement("td");
     ...
     */
    private appendContractMonthCell(
     ...
     */
    private setMonthCellContent(td: HTMLTableCellElement, value: string, hasComments: boolean, commentTooltip: string): void {
     ...
     */
    private createCommentDot(className: string, commentTooltip: string): HTMLSpanElement {
        const dot = document.createElement("span");
     ...
     */
    private showCommentTooltip(anchor: HTMLElement, text: string): void {
    ...
    private formatCommentTooltip(commentTooltip: string): string {
    ...
    private getCommentActionIcon(): string {
        return `
    ...
    private getPaidActionIcon(): string {
        return `
            <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false">
     ...
     */
    private showAddDropdown(e: MouseEvent, subaccount: SubaccountData): void {
     ...
     */
    private createAddDropdownButton(icon: "add" | "standard", label: string, disabled: boolean, onClick?: () => void): HTMLButtonElement {
    ...
    private getAddDropdownIcon(icon: "add" | "standard"): string {
        if (icon === "add") {
            return `
     ...
     */
    private handleAddDropdownWindowClick(e: MouseEvent): void {
     ...
     */
    private showMonthActionDropdown(e: Event, contract: ContractData, monthIndex: number, startYear: number, isPaid: boolean, td: HTMLTableCellElement): void {
     ...
     */
    private isEqualDistribution(distributionType: string): boolean {
     ...
     */
    private handleMonthDropdownWindowClick(e: MouseEvent): void {
        const target = e.target as HTMLElement;
    ...
    private closeMonthDropdown(): void {
    ...
    private renderEmptyState(colSpan: number): void {
        this._tbody.innerHTML = `
            <tr>
                <td colspan="${colSpan}">
