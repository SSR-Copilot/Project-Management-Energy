# Admin Contract Screen migration plan

Plan id: 005-adcontr. Status: **INCOMPLETE - missing standard-assumption sync flow**. This is planning documentation only; it does not implement the migration.

## 1 Purpose

Maintain BoP contract standard assumptions and project-contract defaults by country and CAPEX account. The target is one React/TypeScript Power Apps Code App route, not a re-hosted Canvas screen. The screen must preserve Canvas behavior while replacing Canvas globals, controls, and PCFs with explicit React state, Dataverse services, Fluent UI, and tested rule modules.
...
| Canvas screen YAML | ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/Admin Contract Screen.pa.yaml |
| Existing target feature path | src/features/admin/contract |
| Recommended route | /admin/contract |
...
| Risk | Medium |
| Controls | 145 |
| Formula properties | 2123 |
| Logic blocks reviewed | 88 |
| Substantive logic blocks | 43 |
...
Apply and Apply All Trackings; BoP Contract Standard Assumption DevCo Costs; BoP Contracts Standard Assumptions; BoP Projects Contracts; CAPEX Account Lists; Countries; Fabric Job Types; Fabric Sync Jobs
...
|---|---|---|---|
| ADCONTR-FX-001 | Screen bootstrap and loading gates | ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/Admin Contract Screen.pa.yaml OnVisible, Visible, DisplayMode, and initial collection formulas. | Screen model initializes route state, query keys, permissions, and draft collections before enabling commands. |
| ADCONTR-FX-002 | Project/context filtering | Canvas Filter, LookUp, SortByColumns, and Search formulas over: Apply and Apply All Trackings; BoP Contract Standard Assumption DevCo Costs; BoP Contracts Standard Assumptions; BoP Projects Contracts; CAPEX Account Lists; Countries; Fabric Job Types; Fabric Sync Jobs. | Repository query functions accept typed ids/filters and mirror Canvas row inclusion rules. |
| ADCONTR-FX-003 | Command enablement | Button/icon DisplayMode, command-bar Items, and popup visibility formulas. | rules.ts exposes pure predicates for add/edit/save/delete/submit availability. |
| ADCONTR-FX-004 | Validation and required-field rules | If, IsBlank, Notify, border/color formulas, and save guards. | Validation returns field errors plus blocking summary before any mutation executes. |
| ADCONTR-FX-005 | Patch/remove mutation behavior | Patch, Remove, UpdateIf, and ForAll formulas in behavior properties. | Dataverse service batches equivalent changes with optimistic UI only where Canvas semantics allow it. |
| ADCONTR-FX-006 | Derived numeric/date values | DateAdd, Round, Value, Coalesce, percentage/currency formatting formulas. | Pure calculation helpers preserve Canvas blank/zero/rounding semantics and have table-driven tests. |
| ADCONTR-FX-007 | Local collection lifecycle | ClearCollect, Collect, UpdateContext, and Set formulas. | Local state models Canvas working collections and is reset on project/screen change. |
| ADCONTR-FX-008 | Error/info/confirmation panels | Notify, popup component variables, and confirmation component output formulas. | Shared dialog/toast services expose the same blocking/non-blocking decisions. |
| ADCONTR-FX-009 | Security and role visibility | Role-gated Visible/DisplayMode formulas plus Dataverse role matrix. | Client hides controls for UX, while Dataverse/custom APIs enforce writes. |
| ADCONTR-FX-010 | Navigation and return behavior | Navigate, Back, Launch, and Param formulas. | React Router routes preserve projectId and replace cross-app launch where applicable. |
| ADCONTR-FX-011 | PCF/control event mapping | PowerCAT CommandBar; Creator Kit Icon; FluentDetailsList. | Replace PCF events with typed React component callbacks; preserve payload shape, selection, sorting, and action pulses. |
| ADCONTR-FX-012 | Flow/recalculation side effects | Missing: SynchronizeStandardAssumptionCosts. | Flow calls go through flowClient.ts; missing flows remain implementation blockers until resolved. |
...
PowerCAT CommandBar; Creator Kit Icon; FluentDetailsList
...
Add rule tests for every ADCONTR-FX-* row. Add component tests for load/empty/error/read-only/edit/save/delete paths. Add integration tests around each Dataverse mutation and flow wrapper relevant to this screen. High-risk screens also need regression tests for known Canvas defects or intentional divergences.

## 19 Implementation Files

| Kind | Target |
|---|---|
| Screen component | src/features/admin/contract/Screen.tsx |
| Rules | src/features/admin/contract/rules.ts |
| Rule tests | src/features/admin/contract/rules.test.ts |
| Hooks/model | src/features/admin/contract/hooks.ts or src/features/admin/contract/useScreenModel.ts |
| Repository/service calls | Shared Dataverse services under the target app data layer |
| Route registration | Route config for /admin/contract |

## 20 Existing Code App Gap Notes

Apply/apply-all tracking must remain auditable and cannot be replaced by a pure client mutation. Treat existing React code as a helpful starting point only after comparing it to the Canvas YAML. Where the focused cost app and unified skeleton disagree, prefer Canvas behavior plus the global architecture plan.
...
Risk level: Medium. Dependencies: Admin CAPEX Accounts. Completion status: INCOMPLETE - missing standard-assumption sync flow. Any missing flow, unverified role privilege, or PCF behavior gap must stay visible in the acceptance checklist until closed with source evidence.
...
|---|---|---|
| ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/Admin Contract Screen.pa.yaml | src/features/admin/contract/Screen.tsx | Visual/workflow parity review |
| Canvas formulas counted: 2123 | src/features/admin/contract/rules.ts and hooks | ADCONTR-FX-* tests |
| Logic blocks counted: 88 | Service/model/UI tests | Coverage report below |
| Tables: Apply and Apply All Trackings; BoP Contract Standard Assumption DevCo Costs; BoP Contracts Standard Assumptions; BoP Projects Contracts; CAPEX Account Lists; Countries; Fabric Job Types; Fabric Sync Jobs | Dataverse repositories | Query/mutation tests |
| PCF: PowerCAT CommandBar; Creator Kit Icon; FluentDetailsList | Fluent/React replacement components | Interaction tests |
...
- Inspect ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/Admin Contract Screen.pa.yaml directly before coding.
- Confirm target path src/features/admin/contract exists or create it using existing repo conventions.
- Implement ADCONTR-FX-* rule tests before wiring UI mutations.
- Preserve route /admin/contract and dependency notes.
...
Implement **Admin Contract Screen** as a production Code App screen at /admin/contract using source ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/Admin Contract Screen.pa.yaml. Preserve all behavior represented by 2123 formula properties and 88 logic blocks, prioritizing the 43 substantive behavior blocks.
...
| Input | Value |
|---|---|
| Source YAML | ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/Admin Contract Screen.pa.yaml |
| Tables | Apply and Apply All Trackings; BoP Contract Standard Assumption DevCo Costs; BoP Contracts Standard Assumptions; BoP Projects Contracts; CAPEX Account Lists; Countries; Fabric Job Types; Fabric Sync Jobs |
| PCF/control dependencies | PowerCAT CommandBar; Creator Kit Icon; FluentDetailsList |
| Flow/backend dependencies | Missing: SynchronizeStandardAssumptionCosts |
| Route | /admin/contract |
| Feature path | src/features/admin/contract |
| Rule prefix | ADCONTR |

## Build Outputs

- Screen component, model hook, rules module, and tests under src/features/admin/contract.
- Route registration for /admin/contract.
...
- Test evidence for each ADCONTR-FX-* rule.
...
| Controls inventoried | 145 |
| Formula properties classified | 2123 |
| Logic blocks audited | 88 |
| Substantive logic blocks audited | 43 |
| Stable rule IDs in this plan | 12 |
| Dataverse tables mapped | Apply and Apply All Trackings; BoP Contract Standard Assumption DevCo Costs; BoP Contracts Standard Assumptions; BoP Projects Contracts; CAPEX Account Lists; Countries; Fabric Job Types; Fabric Sync Jobs |
| PCF/control dependencies mapped | PowerCAT CommandBar; Creator Kit Icon; FluentDetailsList |
