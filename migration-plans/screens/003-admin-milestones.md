# Admin Milestones Screen migration plan

Plan id: 003-admile. Status: **INCOMPLETE - missing Fabric recalculation flow reference**. This is planning documentation only; it does not implement the migration.

## 1 Purpose

Maintain milestone standard assumptions by country technology and project context. The target is one React/TypeScript Power Apps Code App route, not a re-hosted Canvas screen. The screen must preserve Canvas behavior while replacing Canvas globals, controls, and PCFs with explicit React state, Dataverse services, Fluent UI, and tested rule modules.
...
| Canvas screen YAML | ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/Admin Milestones Screen.pa.yaml |
| Existing target feature path | src/features/admin/milestones |
| Recommended route | /admin/milestones |
...
Countries; Fabric Job Types; Fabric Sync Jobs; Milestones Standard Assumptions; Projects; Technologies
...
|---|---|---|---|
| ADMILE-FX-001 | Screen bootstrap and loading gates | ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/Admin Milestones Screen.pa.yaml OnVisible, Visible, DisplayMode, and initial collection formulas. | Screen model initializes route state, query keys, permissions, and draft collections before enabling commands. |
| ADMILE-FX-002 | Project/context filtering | Canvas Filter, LookUp, SortByColumns, and Search formulas over: Countries; Fabric Job Types; Fabric Sync Jobs; Milestones Standard Assumptions; Projects; Technologies. | Repository query functions accept typed ids/filters and mirror Canvas row inclusion rules. |
| ADMILE-FX-003 | Command enablement | Button/icon DisplayMode, command-bar Items, and popup visibility formulas. | rules.ts exposes pure predicates for add/edit/save/delete/submit availability. |
| ADMILE-FX-004 | Validation and required-field rules | If, IsBlank, Notify, border/color formulas, and save guards. | Validation returns field errors plus blocking summary before any mutation executes. |
| ADMILE-FX-005 | Patch/remove mutation behavior | Patch, Remove, UpdateIf, and ForAll formulas in behavior properties. | Dataverse service batches equivalent changes with optimistic UI only where Canvas semantics allow it. |
| ADMILE-FX-006 | Derived numeric/date values | DateAdd, Round, Value, Coalesce, percentage/currency formatting formulas. | Pure calculation helpers preserve Canvas blank/zero/rounding semantics and have table-driven tests. |
| ADMILE-FX-007 | Local collection lifecycle | ClearCollect, Collect, UpdateContext, and Set formulas. | Local state models Canvas working collections and is reset on project/screen change. |
| ADMILE-FX-008 | Error/info/confirmation panels | Notify, popup component variables, and confirmation component output formulas. | Shared dialog/toast services expose the same blocking/non-blocking decisions. |
| ADMILE-FX-009 | Security and role visibility | Role-gated Visible/DisplayMode formulas plus Dataverse role matrix. | Client hides controls for UX, while Dataverse/custom APIs enforce writes. |
| ADMILE-FX-010 | Navigation and return behavior | Navigate, Back, Launch, and Param formulas. | React Router routes preserve projectId and replace cross-app launch where applicable. |
| ADMILE-FX-011 | PCF/control event mapping | PowerCAT CommandBar; Creator Kit Icon. | Replace PCF events with typed React component callbacks; preserve payload shape, selection, sorting, and action pulses. |
| ADMILE-FX-012 | Flow/recalculation side effects | Missing: ForCountriestriggerFabricrecalculationsforProjects. | Flow calls go through flowClient.ts; missing flows remain implementation blockers until resolved. |
...
Missing: ForCountriestriggerFabricrecalculationsforProjects
...
Add rule tests for every ADMILE-FX-* row. Add component tests for load/empty/error/read-only/edit/save/delete paths. Add integration tests around each Dataverse mutation and flow wrapper relevant to this screen. High-risk screens also need regression tests for known Canvas defects or intentional divergences.

## 19 Implementation Files

| Kind | Target |
|---|---|
| Screen component | src/features/admin/milestones/Screen.tsx |
| Rules | src/features/admin/milestones/rules.ts |
| Rule tests | src/features/admin/milestones/rules.test.ts |
| Hooks/model | src/features/admin/milestones/hooks.ts or src/features/admin/milestones/useScreenModel.ts |
| Repository/service calls | Shared Dataverse services under the target app data layer |
| Route registration | Route config for /admin/milestones |

## 20 Existing Code App Gap Notes

Dataverse edit behavior is known; final recalculation trigger must be resolved from the tenant. Treat existing React code as a helpful starting point only after comparing it to the Canvas YAML. Where the focused cost app and unified skeleton disagree, prefer Canvas behavior plus the global architecture plan.
...
Risk level: Medium. Dependencies: None. Completion status: INCOMPLETE - missing Fabric recalculation flow reference. Any missing flow, unverified role privilege, or PCF behavior gap must stay visible in the acceptance checklist until closed with source evidence.
...
|---|---|---|
| ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/Admin Milestones Screen.pa.yaml | src/features/admin/milestones/Screen.tsx | Visual/workflow parity review |
| Canvas formulas counted: 1207 | src/features/admin/milestones/rules.ts and hooks | ADMILE-FX-* tests |
| Logic blocks counted: 72 | Service/model/UI tests | Coverage report below |
| Tables: Countries; Fabric Job Types; Fabric Sync Jobs; Milestones Standard Assumptions; Projects; Technologies | Dataverse repositories | Query/mutation tests |
| PCF: PowerCAT CommandBar; Creator Kit Icon | Fluent/React replacement components | Interaction tests |
| Flow: Missing: ForCountriestriggerFabricrecalculationsforProjects | flowClient.ts wrappers/custom APIs | Payload/error tests |
...
- Inspect ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/Admin Milestones Screen.pa.yaml directly before coding.
- Confirm target path src/features/admin/milestones exists or create it using existing repo conventions.
- Implement ADMILE-FX-* rule tests before wiring UI mutations.
- Preserve route /admin/milestones and dependency notes.
- Close every blocker named in INCOMPLETE - missing Fabric recalculation flow reference before declaring production parity.

# CLAUDE IMPLEMENTATION PACKAGE

## Mission

Implement **Admin Milestones Screen** as a production Code App screen at /admin/milestones using source ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/Admin Milestones Screen.pa.yaml. Preserve all behavior represented by 1207 formula properties and 72 logic blocks, prioritizing the 43 substantive behavior blocks.
...
- Do not mark this screen complete while status is INCOMPLETE - missing Fabric recalculation flow reference unless the named blocker is closed.

## Build Inputs

| Input | Value |
|---|---|
| Source YAML | ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/Admin Milestones Screen.pa.yaml |
| Tables | Countries; Fabric Job Types; Fabric Sync Jobs; Milestones Standard Assumptions; Projects; Technologies |
| PCF/control dependencies | PowerCAT CommandBar; Creator Kit Icon |
| Flow/backend dependencies | Missing: ForCountriestriggerFabricrecalculationsforProjects |
| Route | /admin/milestones |
| Feature path | src/features/admin/milestones |
| Rule prefix | ADMILE |

## Build Outputs

- Screen component, model hook, rules module, and tests under src/features/admin/milestones.
- Route registration for /admin/milestones.
...
- Test evidence for each ADMILE-FX-* rule.
...
| Coverage item | Value |
|---|---|
| Screen plan status | INCOMPLETE - missing Fabric recalculation flow reference |
| Source screen included | Yes |
| Controls inventoried | 102 |
| Formula properties classified | 1207 |
| Logic blocks audited | 72 |
| Substantive logic blocks audited | 43 |
| Stable rule IDs in this plan | 12 |
| Dataverse tables mapped | Countries; Fabric Job Types; Fabric Sync Jobs; Milestones Standard Assumptions; Projects; Technologies |
| PCF/control dependencies mapped | PowerCAT CommandBar; Creator Kit Icon |
| Flow dependencies mapped | Missing: ForCountriestriggerFabricrecalculationsforProjects |
| Remaining blockers | INCOMPLETE - missing Fabric recalculation flow reference |
