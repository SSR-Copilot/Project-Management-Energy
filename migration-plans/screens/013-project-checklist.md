# Project General CheckList Screen migration plan

Plan id: 013-chklst. Status: **COMPLETE**. This is planning documentation only; it does not implement the migration.

## 1 Purpose

Manage project checklist items notes approval requests and gate/checklist tracking. The target is one React/TypeScript Power Apps Code App route, not a re-hosted Canvas screen. The screen must preserve Canvas behavior while replacing Canvas globals, controls, and PCFs with explicit React state, Dataverse services, Fluent UI, and tested rule modules.
...
| Canvas screen YAML | ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/Project General CheckList Screen.pa.yaml |
| Existing target feature path | src/features/pm/checklist |
| Recommended route | /projects/:projectId/checklist |
...
Check List Default Approvals; Checklist Country And Technologies; Checklist Notes; Checklist Tracking Notes; Project Checklists; Project Default Approvals; Project Default Checklists; Project State Trackings; Project States; Projects
...
|---|---|---|---|
| CHKLST-FX-001 | Screen bootstrap and loading gates | ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/Project General CheckList Screen.pa.yaml OnVisible, Visible, DisplayMode, and initial collection formulas. | Screen model initializes route state, query keys, permissions, and draft collections before enabling commands. |
| CHKLST-FX-002 | Project/context filtering | Canvas Filter, LookUp, SortByColumns, and Search formulas over: Check List Default Approvals; Checklist Country And Technologies; Checklist Notes; Checklist Tracking Notes; Project Checklists; Project Default Approvals; Project Default Checklists; Project State Trackings; Project States; Projects. | Repository query functions accept typed ids/filters and mirror Canvas row inclusion rules. |
| CHKLST-FX-003 | Command enablement | Button/icon DisplayMode, command-bar Items, and popup visibility formulas. | rules.ts exposes pure predicates for add/edit/save/delete/submit availability. |
| CHKLST-FX-004 | Validation and required-field rules | If, IsBlank, Notify, border/color formulas, and save guards. | Validation returns field errors plus blocking summary before any mutation executes. |
| CHKLST-FX-005 | Patch/remove mutation behavior | Patch, Remove, UpdateIf, and ForAll formulas in behavior properties. | Dataverse service batches equivalent changes with optimistic UI only where Canvas semantics allow it. |
| CHKLST-FX-006 | Derived numeric/date values | DateAdd, Round, Value, Coalesce, percentage/currency formatting formulas. | Pure calculation helpers preserve Canvas blank/zero/rounding semantics and have table-driven tests. |
| CHKLST-FX-007 | Local collection lifecycle | ClearCollect, Collect, UpdateContext, and Set formulas. | Local state models Canvas working collections and is reset on project/screen change. |
| CHKLST-FX-008 | Error/info/confirmation panels | Notify, popup component variables, and confirmation component output formulas. | Shared dialog/toast services expose the same blocking/non-blocking decisions. |
| CHKLST-FX-009 | Security and role visibility | Role-gated Visible/DisplayMode formulas plus Dataverse role matrix. | Client hides controls for UX, while Dataverse/custom APIs enforce writes. |
| CHKLST-FX-010 | Navigation and return behavior | Navigate, Back, Launch, and Param formulas. | React Router routes preserve projectId and replace cross-app launch where applicable. |
| CHKLST-FX-011 | PCF/control event mapping | PowerCAT CommandBar; FluentDetailsList; Creator Kit Icon. | Replace PCF events with typed React component callbacks; preserve payload shape, selection, sorting, and action pulses. |
| CHKLST-FX-012 | Flow/recalculation side effects | ComposeChecklistItems; PerformCommonRequestofCheckListApproval; checklist cancellation via custom API. | Flow calls go through flowClient.ts; missing flows remain implementation blockers until resolved. |
...
ComposeChecklistItems; PerformCommonRequestofCheckListApproval; checklist cancellation via custom API
...
Add rule tests for every CHKLST-FX-* row. Add component tests for load/empty/error/read-only/edit/save/delete paths. Add integration tests around each Dataverse mutation and flow wrapper relevant to this screen. High-risk screens also need regression tests for known Canvas defects or intentional divergences.

## 19 Implementation Files

| Kind | Target |
|---|---|
| Screen component | src/features/pm/checklist/Screen.tsx |
| Rules | src/features/pm/checklist/rules.ts |
| Rule tests | src/features/pm/checklist/rules.test.ts |
| Hooks/model | src/features/pm/checklist/hooks.ts or src/features/pm/checklist/useScreenModel.ts |
| Repository/service calls | Shared Dataverse services under the target app data layer |
| Route registration | Route config for /projects/:projectId/checklist |

## 20 Existing Code App Gap Notes

Approval cancellation is intentionally not a direct flow call in the target. Treat existing React code as a helpful starting point only after comparing it to the Canvas YAML. Where the focused cost app and unified skeleton disagree, prefer Canvas behavior plus the global architecture plan.
...
Risk level: High. Dependencies: Production; Admin Default Checklists; Admin Gates Approvals. Completion status: COMPLETE. Any missing flow, unverified role privilege, or PCF behavior gap must stay visible in the acceptance checklist until closed with source evidence.
...
|---|---|---|
| ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/Project General CheckList Screen.pa.yaml | src/features/pm/checklist/Screen.tsx | Visual/workflow parity review |
| Canvas formulas counted: 2136 | src/features/pm/checklist/rules.ts and hooks | CHKLST-FX-* tests |
| Logic blocks counted: 115 | Service/model/UI tests | Coverage report below |
| Tables: Check List Default Approvals; Checklist Country And Technologies; Checklist Notes; Checklist Tracking Notes; Project Checklists; Project Default Approvals; Project Default Checklists; Project State Trackings; Project States; Projects | Dataverse repositories | Query/mutation tests |
| PCF: PowerCAT CommandBar; FluentDetailsList; Creator Kit Icon | Fluent/React replacement components | Interaction tests |
| Flow: ComposeChecklistItems; PerformCommonRequestofCheckListApproval; checklist cancellation via custom API | flowClient.ts wrappers/custom APIs | Payload/error tests |
...
- Inspect ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/Project General CheckList Screen.pa.yaml directly before coding.
- Confirm target path src/features/pm/checklist exists or create it using existing repo conventions.
- Implement CHKLST-FX-* rule tests before wiring UI mutations.
- Preserve route /projects/:projectId/checklist and dependency notes.
...
Implement **Project General CheckList Screen** as a production Code App screen at /projects/:projectId/checklist using source ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/Project General CheckList Screen.pa.yaml. Preserve all behavior represented by 2136 formula properties and 115 logic blocks, prioritizing the 66 substantive behavior blocks.
...
| Input | Value |
|---|---|
| Source YAML | ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/Project General CheckList Screen.pa.yaml |
| Tables | Check List Default Approvals; Checklist Country And Technologies; Checklist Notes; Checklist Tracking Notes; Project Checklists; Project Default Approvals; Project Default Checklists; Project State Trackings; Project States; Projects |
| PCF/control dependencies | PowerCAT CommandBar; FluentDetailsList; Creator Kit Icon |
| Flow/backend dependencies | ComposeChecklistItems; PerformCommonRequestofCheckListApproval; checklist cancellation via custom API |
| Route | /projects/:projectId/checklist |
| Feature path | src/features/pm/checklist |
| Rule prefix | CHKLST |

## Build Outputs

- Screen component, model hook, rules module, and tests under src/features/pm/checklist.
- Route registration for /projects/:projectId/checklist.
...
- Test evidence for each CHKLST-FX-* rule.
...
| Controls inventoried | 141 |
| Formula properties classified | 2136 |
| Logic blocks audited | 115 |
| Substantive logic blocks audited | 66 |
| Stable rule IDs in this plan | 12 |
| Dataverse tables mapped | Check List Default Approvals; Checklist Country And Technologies; Checklist Notes; Checklist Tracking Notes; Project Checklists; Project Default Approvals; Project Default Checklists; Project State Trackings; Project States; Projects |
| PCF/control dependencies mapped | PowerCAT CommandBar; FluentDetailsList; Creator Kit Icon |
| Flow dependencies mapped | ComposeChecklistItems; PerformCommonRequestofCheckListApproval; checklist cancellation via custom API |
