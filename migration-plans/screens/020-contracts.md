# Contracts Screen migration plan

Plan id: 020-contr. Status: **COMPLETE**. This is planning documentation only; it does not implement the migration.

## 1 Purpose

Maintain project contract costs and contract assumptions associated with CAPEX cost lines. The target is one React/TypeScript Power Apps Code App route, not a re-hosted Canvas screen. The screen must preserve Canvas behavior while replacing Canvas globals, controls, and PCFs with explicit React state, Dataverse services, Fluent UI, and tested rule modules.
...
| Canvas screen YAML | ProjectCosts-CodeApp/_extracted/msapp/projectcosts/Src/Contracts Screen.pa.yaml |
| Existing target feature path | src/features/cost/contracts |
| Recommended route | /costs/:projectId/contracts |
...
BoP Projects Contracts; BoP Contracts Standard Assumptions; CAPEX Account Lists; CAPEX Costs; CAPEX Project Contracts; Countries; Projects
...
|---|---|---|---|
| CONTR-FX-001 | Screen bootstrap and loading gates | ProjectCosts-CodeApp/_extracted/msapp/projectcosts/Src/Contracts Screen.pa.yaml OnVisible, Visible, DisplayMode, and initial collection formulas. | Screen model initializes route state, query keys, permissions, and draft collections before enabling commands. |
| CONTR-FX-002 | Project/context filtering | Canvas Filter, LookUp, SortByColumns, and Search formulas over: BoP Projects Contracts; BoP Contracts Standard Assumptions; CAPEX Account Lists; CAPEX Costs; CAPEX Project Contracts; Countries; Projects. | Repository query functions accept typed ids/filters and mirror Canvas row inclusion rules. |
| CONTR-FX-003 | Command enablement | Button/icon DisplayMode, command-bar Items, and popup visibility formulas. | rules.ts exposes pure predicates for add/edit/save/delete/submit availability. |
| CONTR-FX-004 | Validation and required-field rules | If, IsBlank, Notify, border/color formulas, and save guards. | Validation returns field errors plus blocking summary before any mutation executes. |
| CONTR-FX-005 | Patch/remove mutation behavior | Patch, Remove, UpdateIf, and ForAll formulas in behavior properties. | Dataverse service batches equivalent changes with optimistic UI only where Canvas semantics allow it. |
| CONTR-FX-006 | Derived numeric/date values | DateAdd, Round, Value, Coalesce, percentage/currency formatting formulas. | Pure calculation helpers preserve Canvas blank/zero/rounding semantics and have table-driven tests. |
| CONTR-FX-007 | Local collection lifecycle | ClearCollect, Collect, UpdateContext, and Set formulas. | Local state models Canvas working collections and is reset on project/screen change. |
| CONTR-FX-008 | Error/info/confirmation panels | Notify, popup component variables, and confirmation component output formulas. | Shared dialog/toast services expose the same blocking/non-blocking decisions. |
| CONTR-FX-009 | Security and role visibility | Role-gated Visible/DisplayMode formulas plus Dataverse role matrix. | Client hides controls for UX, while Dataverse/custom APIs enforce writes. |
| CONTR-FX-010 | Navigation and return behavior | Navigate, Back, Launch, and Param formulas. | React Router routes preserve projectId and replace cross-app launch where applicable. |
| CONTR-FX-011 | PCF/control event mapping | PowerCAT CommandBar; FluentDetailsList; Creator Kit Icon. | Replace PCF events with typed React component callbacks; preserve payload shape, selection, sorting, and action pulses. |
| CONTR-FX-012 | Flow/recalculation side effects | LoadDevexCapexCostTotalCapacityTrigger when recalculation is required. | Flow calls go through flowClient.ts; missing flows remain implementation blockers until resolved. |
...
Add rule tests for every CONTR-FX-* row. Add component tests for load/empty/error/read-only/edit/save/delete paths. Add integration tests around each Dataverse mutation and flow wrapper relevant to this screen. High-risk screens also need regression tests for known Canvas defects or intentional divergences.

## 19 Implementation Files

| Kind | Target |
|---|---|
| Screen component | src/features/cost/contracts/Screen.tsx |
| Rules | src/features/cost/contracts/rules.ts |
| Rule tests | src/features/cost/contracts/rules.test.ts |
| Hooks/model | src/features/cost/contracts/hooks.ts or src/features/cost/contracts/useScreenModel.ts |
| Repository/service calls | Shared Dataverse services under the target app data layer |
| Route registration | Route config for /costs/:projectId/contracts |

## 20 Existing Code App Gap Notes

Contract rows must stay consistent with CAPEX account and country defaults. Treat existing React code as a helpful starting point only after comparing it to the Canvas YAML. Where the focused cost app and unified skeleton disagree, prefer Canvas behavior plus the global architecture plan.
...
Risk level: Medium. Dependencies: Capex Costs; Admin Contract. Completion status: COMPLETE. Any missing flow, unverified role privilege, or PCF behavior gap must stay visible in the acceptance checklist until closed with source evidence.
...
|---|---|---|
| ProjectCosts-CodeApp/_extracted/msapp/projectcosts/Src/Contracts Screen.pa.yaml | src/features/cost/contracts/Screen.tsx | Visual/workflow parity review |
| Canvas formulas counted: 3942 | src/features/cost/contracts/rules.ts and hooks | CONTR-FX-* tests |
| Logic blocks counted: 149 | Service/model/UI tests | Coverage report below |
| Tables: BoP Projects Contracts; BoP Contracts Standard Assumptions; CAPEX Account Lists; CAPEX Costs; CAPEX Project Contracts; Countries; Projects | Dataverse repositories | Query/mutation tests |
...
- Inspect ProjectCosts-CodeApp/_extracted/msapp/projectcosts/Src/Contracts Screen.pa.yaml directly before coding.
- Confirm target path src/features/cost/contracts exists or create it using existing repo conventions.
- Implement CONTR-FX-* rule tests before wiring UI mutations.
- Preserve route /costs/:projectId/contracts and dependency notes.
...
Implement **Contracts Screen** as a production Code App screen at /costs/:projectId/contracts using source ProjectCosts-CodeApp/_extracted/msapp/projectcosts/Src/Contracts Screen.pa.yaml. Preserve all behavior represented by 3942 formula properties and 149 logic blocks, prioritizing the 80 substantive behavior blocks.
...
| Input | Value |
|---|---|
| Source YAML | ProjectCosts-CodeApp/_extracted/msapp/projectcosts/Src/Contracts Screen.pa.yaml |
| Tables | BoP Projects Contracts; BoP Contracts Standard Assumptions; CAPEX Account Lists; CAPEX Costs; CAPEX Project Contracts; Countries; Projects |
...
| Route | /costs/:projectId/contracts |
| Feature path | src/features/cost/contracts |
| Rule prefix | CONTR |

## Build Outputs

- Screen component, model hook, rules module, and tests under src/features/cost/contracts.
- Route registration for /costs/:projectId/contracts.
...
- Test evidence for each CONTR-FX-* rule.
...
| Controls inventoried | 243 |
| Formula properties classified | 3942 |
| Logic blocks audited | 149 |
| Substantive logic blocks audited | 80 |
| Stable rule IDs in this plan | 12 |
| Dataverse tables mapped | BoP Projects Contracts; BoP Contracts Standard Assumptions; CAPEX Account Lists; CAPEX Costs; CAPEX Project Contracts; Countries; Projects |
