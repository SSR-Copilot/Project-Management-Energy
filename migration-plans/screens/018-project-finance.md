# Project Finance Screen migration plan

Plan id: 018-fin. Status: **COMPLETE**. This is planning documentation only; it does not implement the migration.

## 1 Purpose

Maintain financing inputs repayment amounts margins generator-linked finance values and derived financial metrics. The target is one React/TypeScript Power Apps Code App route, not a re-hosted Canvas screen. The screen must preserve Canvas behavior while replacing Canvas globals, controls, and PCFs with explicit React state, Dataverse services, Fluent UI, and tested rule modules.
...
| Canvas screen YAML | ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/Project Finance Screen.pa.yaml |
| Existing target feature path | src/features/pm/finance |
| Recommended route | /projects/:projectId/finance |
...
CAPEX Costs; Financing Input Step Up margins; Financing Inputs; Financing Inputs Repayment Amounts; GeneratorInProjects; GeneratorTypeInProjects; Generators; Projects
...
|---|---|---|---|
| FIN-FX-001 | Screen bootstrap and loading gates | ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/Project Finance Screen.pa.yaml OnVisible, Visible, DisplayMode, and initial collection formulas. | Screen model initializes route state, query keys, permissions, and draft collections before enabling commands. |
| FIN-FX-002 | Project/context filtering | Canvas Filter, LookUp, SortByColumns, and Search formulas over: CAPEX Costs; Financing Input Step Up margins; Financing Inputs; Financing Inputs Repayment Amounts; GeneratorInProjects; GeneratorTypeInProjects; Generators; Projects. | Repository query functions accept typed ids/filters and mirror Canvas row inclusion rules. |
| FIN-FX-003 | Command enablement | Button/icon DisplayMode, command-bar Items, and popup visibility formulas. | rules.ts exposes pure predicates for add/edit/save/delete/submit availability. |
| FIN-FX-004 | Validation and required-field rules | If, IsBlank, Notify, border/color formulas, and save guards. | Validation returns field errors plus blocking summary before any mutation executes. |
| FIN-FX-005 | Patch/remove mutation behavior | Patch, Remove, UpdateIf, and ForAll formulas in behavior properties. | Dataverse service batches equivalent changes with optimistic UI only where Canvas semantics allow it. |
| FIN-FX-006 | Derived numeric/date values | DateAdd, Round, Value, Coalesce, percentage/currency formatting formulas. | Pure calculation helpers preserve Canvas blank/zero/rounding semantics and have table-driven tests. |
| FIN-FX-007 | Local collection lifecycle | ClearCollect, Collect, UpdateContext, and Set formulas. | Local state models Canvas working collections and is reset on project/screen change. |
| FIN-FX-008 | Error/info/confirmation panels | Notify, popup component variables, and confirmation component output formulas. | Shared dialog/toast services expose the same blocking/non-blocking decisions. |
| FIN-FX-009 | Security and role visibility | Role-gated Visible/DisplayMode formulas plus Dataverse role matrix. | Client hides controls for UX, while Dataverse/custom APIs enforce writes. |
| FIN-FX-010 | Navigation and return behavior | Navigate, Back, Launch, and Param formulas. | React Router routes preserve projectId and replace cross-app launch where applicable. |
| FIN-FX-011 | PCF/control event mapping | PowerCAT CommandBar; FluentDetailsList; Creator Kit Icon. | Replace PCF events with typed React component callbacks; preserve payload shape, selection, sorting, and action pulses. |
| FIN-FX-012 | Flow/recalculation side effects | None active. | Flow calls go through flowClient.ts; missing flows remain implementation blockers until resolved. |
...
Add rule tests for every FIN-FX-* row. Add component tests for load/empty/error/read-only/edit/save/delete paths. Add integration tests around each Dataverse mutation and flow wrapper relevant to this screen. High-risk screens also need regression tests for known Canvas defects or intentional divergences.

## 19 Implementation Files

| Kind | Target |
|---|---|
| Screen component | src/features/pm/finance/Screen.tsx |
| Rules | src/features/pm/finance/rules.ts |
| Rule tests | src/features/pm/finance/rules.test.ts |
| Hooks/model | src/features/pm/finance/hooks.ts or src/features/pm/finance/useScreenModel.ts |
| Repository/service calls | Shared Dataverse services under the target app data layer |
| Route registration | Route config for /projects/:projectId/finance |

## 20 Existing Code App Gap Notes

Documented Debt Financing overwrite defect must be addressed deliberately with tests. Treat existing React code as a helpful starting point only after comparing it to the Canvas YAML. Where the focused cost app and unified skeleton disagree, prefer Canvas behavior plus the global architecture plan.
...
Risk level: High. Dependencies: Revenues. Completion status: COMPLETE. Any missing flow, unverified role privilege, or PCF behavior gap must stay visible in the acceptance checklist until closed with source evidence.
...
|---|---|---|
| ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/Project Finance Screen.pa.yaml | src/features/pm/finance/Screen.tsx | Visual/workflow parity review |
| Canvas formulas counted: 6973 | src/features/pm/finance/rules.ts and hooks | FIN-FX-* tests |
| Logic blocks counted: 321 | Service/model/UI tests | Coverage report below |
| Tables: CAPEX Costs; Financing Input Step Up margins; Financing Inputs; Financing Inputs Repayment Amounts; GeneratorInProjects; GeneratorTypeInProjects; Generators; Projects | Dataverse repositories | Query/mutation tests |
| PCF: PowerCAT CommandBar; FluentDetailsList; Creator Kit Icon | Fluent/React replacement components | Interaction tests |
| Flow: None active | flowClient.ts wrappers/custom APIs | Payload/error tests |
...
- Inspect ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/Project Finance Screen.pa.yaml directly before coding.
- Confirm target path src/features/pm/finance exists or create it using existing repo conventions.
- Implement FIN-FX-* rule tests before wiring UI mutations.
- Preserve route /projects/:projectId/finance and dependency notes.
...
Implement **Project Finance Screen** as a production Code App screen at /projects/:projectId/finance using source ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/Project Finance Screen.pa.yaml. Preserve all behavior represented by 6973 formula properties and 321 logic blocks, prioritizing the 186 substantive behavior blocks.
...
| Input | Value |
|---|---|
| Source YAML | ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/Project Finance Screen.pa.yaml |
| Tables | CAPEX Costs; Financing Input Step Up margins; Financing Inputs; Financing Inputs Repayment Amounts; GeneratorInProjects; GeneratorTypeInProjects; Generators; Projects |
| PCF/control dependencies | PowerCAT CommandBar; FluentDetailsList; Creator Kit Icon |
| Flow/backend dependencies | None active |
| Route | /projects/:projectId/finance |
| Feature path | src/features/pm/finance |
| Rule prefix | FIN |

## Build Outputs

- Screen component, model hook, rules module, and tests under src/features/pm/finance.
- Route registration for /projects/:projectId/finance.
...
- Test evidence for each FIN-FX-* rule.
...
| Controls inventoried | 604 |
| Formula properties classified | 6973 |
| Logic blocks audited | 321 |
| Substantive logic blocks audited | 186 |
| Stable rule IDs in this plan | 12 |
| Dataverse tables mapped | CAPEX Costs; Financing Input Step Up margins; Financing Inputs; Financing Inputs Repayment Amounts; GeneratorInProjects; GeneratorTypeInProjects; Generators; Projects |
| PCF/control dependencies mapped | PowerCAT CommandBar; FluentDetailsList; Creator Kit Icon |
| Flow dependencies mapped | None active |
