# Grid Operator Screen migration plan

Plan id: 016-gridop. Status: **COMPLETE**. This is planning documentation only; it does not implement the migration.

## 1 Purpose

Maintain grid operator information linked to the selected project. The target is one React/TypeScript Power Apps Code App route, not a re-hosted Canvas screen. The screen must preserve Canvas behavior while replacing Canvas globals, controls, and PCFs with explicit React state, Dataverse services, Fluent UI, and tested rule modules.
...
| Canvas screen YAML | ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/Grid Operator Screen.pa.yaml |
| Existing target feature path | src/features/pm/grid-operator |
| Recommended route | /projects/:projectId/grid-operator |
...
|---|---|---|---|
| GRIDOP-FX-001 | Screen bootstrap and loading gates | ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/Grid Operator Screen.pa.yaml OnVisible, Visible, DisplayMode, and initial collection formulas. | Screen model initializes route state, query keys, permissions, and draft collections before enabling commands. |
| GRIDOP-FX-002 | Project/context filtering | Canvas Filter, LookUp, SortByColumns, and Search formulas over: Grid Operators; Projects. | Repository query functions accept typed ids/filters and mirror Canvas row inclusion rules. |
| GRIDOP-FX-003 | Command enablement | Button/icon DisplayMode, command-bar Items, and popup visibility formulas. | rules.ts exposes pure predicates for add/edit/save/delete/submit availability. |
| GRIDOP-FX-004 | Validation and required-field rules | If, IsBlank, Notify, border/color formulas, and save guards. | Validation returns field errors plus blocking summary before any mutation executes. |
| GRIDOP-FX-005 | Patch/remove mutation behavior | Patch, Remove, UpdateIf, and ForAll formulas in behavior properties. | Dataverse service batches equivalent changes with optimistic UI only where Canvas semantics allow it. |
| GRIDOP-FX-006 | Derived numeric/date values | DateAdd, Round, Value, Coalesce, percentage/currency formatting formulas. | Pure calculation helpers preserve Canvas blank/zero/rounding semantics and have table-driven tests. |
| GRIDOP-FX-007 | Local collection lifecycle | ClearCollect, Collect, UpdateContext, and Set formulas. | Local state models Canvas working collections and is reset on project/screen change. |
| GRIDOP-FX-008 | Error/info/confirmation panels | Notify, popup component variables, and confirmation component output formulas. | Shared dialog/toast services expose the same blocking/non-blocking decisions. |
| GRIDOP-FX-009 | Security and role visibility | Role-gated Visible/DisplayMode formulas plus Dataverse role matrix. | Client hides controls for UX, while Dataverse/custom APIs enforce writes. |
| GRIDOP-FX-010 | Navigation and return behavior | Navigate, Back, Launch, and Param formulas. | React Router routes preserve projectId and replace cross-app launch where applicable. |
| GRIDOP-FX-011 | PCF/control event mapping | PowerCAT CommandBar; Creator Kit Icon. | Replace PCF events with typed React component callbacks; preserve payload shape, selection, sorting, and action pulses. |
| GRIDOP-FX-012 | Flow/recalculation side effects | None. | Flow calls go through flowClient.ts; missing flows remain implementation blockers until resolved. |
...
Add rule tests for every GRIDOP-FX-* row. Add component tests for load/empty/error/read-only/edit/save/delete paths. Add integration tests around each Dataverse mutation and flow wrapper relevant to this screen. High-risk screens also need regression tests for known Canvas defects or intentional divergences.

## 19 Implementation Files

| Kind | Target |
|---|---|
| Screen component | src/features/pm/grid-operator/Screen.tsx |
| Rules | src/features/pm/grid-operator/rules.ts |
| Rule tests | src/features/pm/grid-operator/rules.test.ts |
| Hooks/model | src/features/pm/grid-operator/hooks.ts or src/features/pm/grid-operator/useScreenModel.ts |
| Repository/service calls | Shared Dataverse services under the target app data layer |
| Route registration | Route config for /projects/:projectId/grid-operator |

## 20 Existing Code App Gap Notes

Use the existing guarded-mutation pattern from the unified skeleton. Treat existing React code as a helpful starting point only after comparing it to the Canvas YAML. Where the focused cost app and unified skeleton disagree, prefer Canvas behavior plus the global architecture plan.
...
|---|---|---|
| ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/Grid Operator Screen.pa.yaml | src/features/pm/grid-operator/Screen.tsx | Visual/workflow parity review |
| Canvas formulas counted: 743 | src/features/pm/grid-operator/rules.ts and hooks | GRIDOP-FX-* tests |
| Logic blocks counted: 47 | Service/model/UI tests | Coverage report below |
| Tables: Grid Operators; Projects | Dataverse repositories | Query/mutation tests |
...
- Inspect ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/Grid Operator Screen.pa.yaml directly before coding.
- Confirm target path src/features/pm/grid-operator exists or create it using existing repo conventions.
- Implement GRIDOP-FX-* rule tests before wiring UI mutations.
- Preserve route /projects/:projectId/grid-operator and dependency notes.
...
Implement **Grid Operator Screen** as a production Code App screen at /projects/:projectId/grid-operator using source ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/Grid Operator Screen.pa.yaml. Preserve all behavior represented by 743 formula properties and 47 logic blocks, prioritizing the 23 substantive behavior blocks.
...
| Input | Value |
|---|---|
| Source YAML | ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/Grid Operator Screen.pa.yaml |
| Tables | Grid Operators; Projects |
| PCF/control dependencies | PowerCAT CommandBar; Creator Kit Icon |
| Flow/backend dependencies | None |
| Route | /projects/:projectId/grid-operator |
| Feature path | src/features/pm/grid-operator |
| Rule prefix | GRIDOP |

## Build Outputs

- Screen component, model hook, rules module, and tests under src/features/pm/grid-operator.
- Route registration for /projects/:projectId/grid-operator.
...
- Test evidence for each GRIDOP-FX-* rule.
...
| Controls inventoried | 68 |
| Formula properties classified | 743 |
| Logic blocks audited | 47 |
| Substantive logic blocks audited | 23 |
| Stable rule IDs in this plan | 12 |
| Dataverse tables mapped | Grid Operators; Projects |
