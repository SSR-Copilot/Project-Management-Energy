...
| Canvas screen YAML | ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/Project Revenues Screen.pa.yaml |
| Existing target feature path | src/features/pm/revenues |
| Recommended route | /projects/:projectId/revenues |
...
Balancing Prices; Country Inflation Profiles; Energy Yields; Project Revenues; Projects; RevenueIndividualHedgeVolumes; Tarrif Price Standard Assumptions
...
|---|---|---|---|
| REV-FX-001 | Screen bootstrap and loading gates | ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/Project Revenues Screen.pa.yaml OnVisible, Visible, DisplayMode, and initial collection formulas. | Screen model initializes route state, query keys, permissions, and draft collections before enabling commands. |
| REV-FX-002 | Project/context filtering | Canvas Filter, LookUp, SortByColumns, and Search formulas over: Balancing Prices; Country Inflation Profiles; Energy Yields; Project Revenues; Projects; RevenueIndividualHedgeVolumes; Tarrif Price Standard Assumptions. | Repository query functions accept typed ids/filters and mirror Canvas row inclusion rules. |
| REV-FX-003 | Command enablement | Button/icon DisplayMode, command-bar Items, and popup visibility formulas. | rules.ts exposes pure predicates for add/edit/save/delete/submit availability. |
| REV-FX-004 | Validation and required-field rules | If, IsBlank, Notify, border/color formulas, and save guards. | Validation returns field errors plus blocking summary before any mutation executes. |
| REV-FX-005 | Patch/remove mutation behavior | Patch, Remove, UpdateIf, and ForAll formulas in behavior properties. | Dataverse service batches equivalent changes with optimistic UI only where Canvas semantics allow it. |
| REV-FX-006 | Derived numeric/date values | DateAdd, Round, Value, Coalesce, percentage/currency formatting formulas. | Pure calculation helpers preserve Canvas blank/zero/rounding semantics and have table-driven tests. |
| REV-FX-007 | Local collection lifecycle | ClearCollect, Collect, UpdateContext, and Set formulas. | Local state models Canvas working collections and is reset on project/screen change. |
| REV-FX-008 | Error/info/confirmation panels | Notify, popup component variables, and confirmation component output formulas. | Shared dialog/toast services expose the same blocking/non-blocking decisions. |
| REV-FX-009 | Security and role visibility | Role-gated Visible/DisplayMode formulas plus Dataverse role matrix. | Client hides controls for UX, while Dataverse/custom APIs enforce writes. |
| REV-FX-010 | Navigation and return behavior | Navigate, Back, Launch, and Param formulas. | React Router routes preserve projectId and replace cross-app launch where applicable. |
| REV-FX-011 | PCF/control event mapping | PowerCAT CommandBar; FluentDetailsList; Creator Kit Icon. | Replace PCF events with typed React component callbacks; preserve payload shape, selection, sorting, and action pulses. |
| REV-FX-012 | Flow/recalculation side effects | None active. | Flow calls go through flowClient.ts; missing flows remain implementation blockers until resolved. |
...
Add rule tests for every REV-FX-* row. Add component tests for load/empty/error/read-only/edit/save/delete paths. Add integration tests around each Dataverse mutation and flow wrapper relevant to this screen. High-risk screens also need regression tests for known Canvas defects or intentional divergences.

## 19 Implementation Files

| Kind | Target |
|---|---|
| Screen component | src/features/pm/revenues/Screen.tsx |
| Rules | src/features/pm/revenues/rules.ts |
| Rule tests | src/features/pm/revenues/rules.test.ts |
| Hooks/model | src/features/pm/revenues/hooks.ts or src/features/pm/revenues/useScreenModel.ts |
| Repository/service calls | Shared Dataverse services under the target app data layer |
| Route registration | Route config for /projects/:projectId/revenues |

## 20 Existing Code App Gap Notes

Preserve the documented Canvas price-fallback defect only if product confirms. Treat existing React code as a helpful starting point only after comparing it to the Canvas YAML. Where the focused cost app and unified skeleton disagree, prefer Canvas behavior plus the global architecture plan.
...
Risk level: Medium. Dependencies: Production; Admin Cost. Completion status: COMPLETE. Any missing flow, unverified role privilege, or PCF behavior gap must stay visible in the acceptance checklist until closed with source evidence.
...
|---|---|---|
| ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/Project Revenues Screen.pa.yaml | src/features/pm/revenues/Screen.tsx | Visual/workflow parity review |
| Canvas formulas counted: 4689 | src/features/pm/revenues/rules.ts and hooks | REV-FX-* tests |
| Logic blocks counted: 216 | Service/model/UI tests | Coverage report below |
| Tables: Balancing Prices; Country Inflation Profiles; Energy Yields; Project Revenues; Projects; RevenueIndividualHedgeVolumes; Tarrif Price Standard Assumptions | Dataverse repositories | Query/mutation tests |
...
- Inspect ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/Project Revenues Screen.pa.yaml directly before coding.
- Confirm target path src/features/pm/revenues exists or create it using existing repo conventions.
- Implement REV-FX-* rule tests before wiring UI mutations.
- Preserve route /projects/:projectId/revenues and dependency notes.
...
Implement **Project Revenues Screen** as a production Code App screen at /projects/:projectId/revenues using source ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/Project Revenues Screen.pa.yaml. Preserve all behavior represented by 4689 formula properties and 216 logic blocks, prioritizing the 122 substantive behavior blocks.
...
| Input | Value |
|---|---|
| Source YAML | ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/Project Revenues Screen.pa.yaml |
| Tables | Balancing Prices; Country Inflation Profiles; Energy Yields; Project Revenues; Projects; RevenueIndividualHedgeVolumes; Tarrif Price Standard Assumptions |
| PCF/control dependencies | PowerCAT CommandBar; FluentDetailsList; Creator Kit Icon |
| Flow/backend dependencies | None active |
| Route | /projects/:projectId/revenues |
| Feature path | src/features/pm/revenues |
| Rule prefix | REV |

## Build Outputs

- Screen component, model hook, rules module, and tests under src/features/pm/revenues.
- Route registration for /projects/:projectId/revenues.
...
- Test evidence for each REV-FX-* rule.
...
| Controls inventoried | 289 |
| Formula properties classified | 4689 |
| Logic blocks audited | 216 |
| Substantive logic blocks audited | 122 |
| Stable rule IDs in this plan | 12 |
| Dataverse tables mapped | Balancing Prices; Country Inflation Profiles; Energy Yields; Project Revenues; Projects; RevenueIndividualHedgeVolumes; Tarrif Price Standard Assumptions |
