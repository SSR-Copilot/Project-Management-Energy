# Add Costs from Table migration plan

Plan id: 023-addcost. Status: **COMPLETE**. This is planning documentation only; it does not implement the migration.

## 1 Purpose

Import group edit paste add delete and save cost rows from a spreadsheet-like PCF surface. The target is one React/TypeScript Power Apps Code App route, not a re-hosted Canvas screen. The screen must preserve Canvas behavior while replacing Canvas globals, controls, and PCFs with explicit React state, Dataverse services, Fluent UI, and tested rule modules.
...
| Canvas screen YAML | ProjectCosts-CodeApp/_extracted/msapp/projectcosts/Src/Add Costs from Table.pa.yaml |
| Existing target feature path | src/features/cost/add-costs-from-table |
| Recommended route | /costs/:projectId/add-from-table |
...
CAPEX Costs; CAPEX Account Lists; Projects
...
|---|---|---|---|
| ADDCOST-FX-001 | Screen bootstrap and loading gates | ProjectCosts-CodeApp/_extracted/msapp/projectcosts/Src/Add Costs from Table.pa.yaml OnVisible, Visible, DisplayMode, and initial collection formulas. | Screen model initializes route state, query keys, permissions, and draft collections before enabling commands. |
| ADDCOST-FX-002 | Project/context filtering | Canvas Filter, LookUp, SortByColumns, and Search formulas over: CAPEX Costs; CAPEX Account Lists; Projects. | Repository query functions accept typed ids/filters and mirror Canvas row inclusion rules. |
| ADDCOST-FX-003 | Command enablement | Button/icon DisplayMode, command-bar Items, and popup visibility formulas. | rules.ts exposes pure predicates for add/edit/save/delete/submit availability. |
| ADDCOST-FX-004 | Validation and required-field rules | If, IsBlank, Notify, border/color formulas, and save guards. | Validation returns field errors plus blocking summary before any mutation executes. |
| ADDCOST-FX-005 | Patch/remove mutation behavior | Patch, Remove, UpdateIf, and ForAll formulas in behavior properties. | Dataverse service batches equivalent changes with optimistic UI only where Canvas semantics allow it. |
| ADDCOST-FX-006 | Derived numeric/date values | DateAdd, Round, Value, Coalesce, percentage/currency formatting formulas. | Pure calculation helpers preserve Canvas blank/zero/rounding semantics and have table-driven tests. |
| ADDCOST-FX-007 | Local collection lifecycle | ClearCollect, Collect, UpdateContext, and Set formulas. | Local state models Canvas working collections and is reset on project/screen change. |
| ADDCOST-FX-008 | Error/info/confirmation panels | Notify, popup component variables, and confirmation component output formulas. | Shared dialog/toast services expose the same blocking/non-blocking decisions. |
| ADDCOST-FX-009 | Security and role visibility | Role-gated Visible/DisplayMode formulas plus Dataverse role matrix. | Client hides controls for UX, while Dataverse/custom APIs enforce writes. |
| ADDCOST-FX-010 | Navigation and return behavior | Navigate, Back, Launch, and Param formulas. | React Router routes preserve projectId and replace cross-app launch where applicable. |
| ADDCOST-FX-011 | PCF/control event mapping | vsb_Dev.SpreadSheet. | Replace PCF events with typed React component callbacks; preserve payload shape, selection, sorting, and action pulses. |
| ADDCOST-FX-012 | Flow/recalculation side effects | None active. | Flow calls go through flowClient.ts; missing flows remain implementation blockers until resolved. |
...
Add rule tests for every ADDCOST-FX-* row. Add component tests for load/empty/error/read-only/edit/save/delete paths. Add integration tests around each Dataverse mutation and flow wrapper relevant to this screen. High-risk screens also need regression tests for known Canvas defects or intentional divergences.

## 19 Implementation Files

| Kind | Target |
|---|---|
| Screen component | src/features/cost/add-costs-from-table/Screen.tsx |
| Rules | src/features/cost/add-costs-from-table/rules.ts |
| Rule tests | src/features/cost/add-costs-from-table/rules.test.ts |
| Hooks/model | src/features/cost/add-costs-from-table/hooks.ts or src/features/cost/add-costs-from-table/useScreenModel.ts |
| Repository/service calls | Shared Dataverse services under the target app data layer |
| Route registration | Route config for /costs/:projectId/add-from-table |

## 20 Existing Code App Gap Notes

Do not port Canvas timer polling literally; use React state events from the replacement spreadsheet component. Treat existing React code as a helpful starting point only after comparing it to the Canvas YAML. Where the focused cost app and unified skeleton disagree, prefer Canvas behavior plus the global architecture plan.
...
Risk level: Medium. Dependencies: Capex Costs. Completion status: COMPLETE. Any missing flow, unverified role privilege, or PCF behavior gap must stay visible in the acceptance checklist until closed with source evidence.
...
|---|---|---|
| ProjectCosts-CodeApp/_extracted/msapp/projectcosts/Src/Add Costs from Table.pa.yaml | src/features/cost/add-costs-from-table/Screen.tsx | Visual/workflow parity review |
| Canvas formulas counted: 224 | src/features/cost/add-costs-from-table/rules.ts and hooks | ADDCOST-FX-* tests |
| Logic blocks counted: 9 | Service/model/UI tests | Coverage report below |
| Tables: CAPEX Costs; CAPEX Account Lists; Projects | Dataverse repositories | Query/mutation tests |
| PCF: vsb_Dev.SpreadSheet | Fluent/React replacement components | Interaction tests |
...
- Inspect ProjectCosts-CodeApp/_extracted/msapp/projectcosts/Src/Add Costs from Table.pa.yaml directly before coding.
- Confirm target path src/features/cost/add-costs-from-table exists or create it using existing repo conventions.
- Implement ADDCOST-FX-* rule tests before wiring UI mutations.
- Preserve route /costs/:projectId/add-from-table and dependency notes.
...
Implement **Add Costs from Table** as a production Code App screen at /costs/:projectId/add-from-table using source ProjectCosts-CodeApp/_extracted/msapp/projectcosts/Src/Add Costs from Table.pa.yaml. Preserve all behavior represented by 224 formula properties and 9 logic blocks, prioritizing the 3 substantive behavior blocks.
...
| Input | Value |
|---|---|
| Source YAML | ProjectCosts-CodeApp/_extracted/msapp/projectcosts/Src/Add Costs from Table.pa.yaml |
| Tables | CAPEX Costs; CAPEX Account Lists; Projects |
| PCF/control dependencies | vsb_Dev.SpreadSheet |
| Flow/backend dependencies | None active |
| Route | /costs/:projectId/add-from-table |
| Feature path | src/features/cost/add-costs-from-table |
| Rule prefix | ADDCOST |

## Build Outputs

- Screen component, model hook, rules module, and tests under src/features/cost/add-costs-from-table.
- Route registration for /costs/:projectId/add-from-table.
...
- Test evidence for each ADDCOST-FX-* rule.
...
| Controls inventoried | 23 |
| Formula properties classified | 224 |
| Logic blocks audited | 9 |
| Substantive logic blocks audited | 3 |
| Stable rule IDs in this plan | 12 |
| Dataverse tables mapped | CAPEX Costs; CAPEX Account Lists; Projects |
| PCF/control dependencies mapped | vsb_Dev.SpreadSheet |
