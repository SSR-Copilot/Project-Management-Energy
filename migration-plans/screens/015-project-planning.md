# Project Planning Screen migration plan

Plan id: 015-plan. Status: **COMPLETE**. This is planning documentation only; it does not implement the migration.

## 1 Purpose

Maintain acquisition status acquisition type project planning fields and permit-linked planning values. The target is one React/TypeScript Power Apps Code App route, not a re-hosted Canvas screen. The screen must preserve Canvas behavior while replacing Canvas globals, controls, and PCFs with explicit React state, Dataverse services, Fluent UI, and tested rule modules.
...
| Canvas screen YAML | ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/Project Planning Screen.pa.yaml |
| Existing target feature path | src/features/pm/planning |
| Recommended route | /projects/:projectId/planning |
...
| Risk | Medium |
| Controls | 178 |
| Formula properties | 2050 |
| Logic blocks reviewed | 84 |
...
Aquisition Statuses; Aquisition Types; Custom Choice Values; Permits; Project Plannings; Projects
...
|---|---|---|---|
| PLAN-FX-001 | Screen bootstrap and loading gates | ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/Project Planning Screen.pa.yaml OnVisible, Visible, DisplayMode, and initial collection formulas. | Screen model initializes route state, query keys, permissions, and draft collections before enabling commands. |
| PLAN-FX-002 | Project/context filtering | Canvas Filter, LookUp, SortByColumns, and Search formulas over: Aquisition Statuses; Aquisition Types; Custom Choice Values; Permits; Project Plannings; Projects. | Repository query functions accept typed ids/filters and mirror Canvas row inclusion rules. |
| PLAN-FX-003 | Command enablement | Button/icon DisplayMode, command-bar Items, and popup visibility formulas. | rules.ts exposes pure predicates for add/edit/save/delete/submit availability. |
| PLAN-FX-004 | Validation and required-field rules | If, IsBlank, Notify, border/color formulas, and save guards. | Validation returns field errors plus blocking summary before any mutation executes. |
| PLAN-FX-005 | Patch/remove mutation behavior | Patch, Remove, UpdateIf, and ForAll formulas in behavior properties. | Dataverse service batches equivalent changes with optimistic UI only where Canvas semantics allow it. |
| PLAN-FX-006 | Derived numeric/date values | DateAdd, Round, Value, Coalesce, percentage/currency formatting formulas. | Pure calculation helpers preserve Canvas blank/zero/rounding semantics and have table-driven tests. |
| PLAN-FX-007 | Local collection lifecycle | ClearCollect, Collect, UpdateContext, and Set formulas. | Local state models Canvas working collections and is reset on project/screen change. |
| PLAN-FX-008 | Error/info/confirmation panels | Notify, popup component variables, and confirmation component output formulas. | Shared dialog/toast services expose the same blocking/non-blocking decisions. |
| PLAN-FX-009 | Security and role visibility | Role-gated Visible/DisplayMode formulas plus Dataverse role matrix. | Client hides controls for UX, while Dataverse/custom APIs enforce writes. |
| PLAN-FX-010 | Navigation and return behavior | Navigate, Back, Launch, and Param formulas. | React Router routes preserve projectId and replace cross-app launch where applicable. |
| PLAN-FX-011 | PCF/control event mapping | PowerCAT CommandBar; FluentDetailsList; Creator Kit Icon. | Replace PCF events with typed React component callbacks; preserve payload shape, selection, sorting, and action pulses. |
| PLAN-FX-012 | Flow/recalculation side effects | None. | Flow calls go through flowClient.ts; missing flows remain implementation blockers until resolved. |
...
Add rule tests for every PLAN-FX-* row. Add component tests for load/empty/error/read-only/edit/save/delete paths. Add integration tests around each Dataverse mutation and flow wrapper relevant to this screen. High-risk screens also need regression tests for known Canvas defects or intentional divergences.

## 19 Implementation Files

| Kind | Target |
|---|---|
| Screen component | src/features/pm/planning/Screen.tsx |
| Rules | src/features/pm/planning/rules.ts |
| Rule tests | src/features/pm/planning/rules.test.ts |
| Hooks/model | src/features/pm/planning/hooks.ts or src/features/pm/planning/useScreenModel.ts |
| Repository/service calls | Shared Dataverse services under the target app data layer |
| Route registration | Route config for /projects/:projectId/planning |

## 20 Existing Code App Gap Notes

Canvas spelling of Aquisition is preserved in Dataverse display names. Treat existing React code as a helpful starting point only after comparing it to the Canvas YAML. Where the focused cost app and unified skeleton disagree, prefer Canvas behavior plus the global architecture plan.
...
Risk level: Medium. Dependencies: General Data. Completion status: COMPLETE. Any missing flow, unverified role privilege, or PCF behavior gap must stay visible in the acceptance checklist until closed with source evidence.
...
|---|---|---|
| ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/Project Planning Screen.pa.yaml | src/features/pm/planning/Screen.tsx | Visual/workflow parity review |
| Canvas formulas counted: 2050 | src/features/pm/planning/rules.ts and hooks | PLAN-FX-* tests |
| Logic blocks counted: 84 | Service/model/UI tests | Coverage report below |
| Tables: Aquisition Statuses; Aquisition Types; Custom Choice Values; Permits; Project Plannings; Projects | Dataverse repositories | Query/mutation tests |
...
- Inspect ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/Project Planning Screen.pa.yaml directly before coding.
- Confirm target path src/features/pm/planning exists or create it using existing repo conventions.
- Implement PLAN-FX-* rule tests before wiring UI mutations.
- Preserve route /projects/:projectId/planning and dependency notes.
...
Implement **Project Planning Screen** as a production Code App screen at /projects/:projectId/planning using source ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/Project Planning Screen.pa.yaml. Preserve all behavior represented by 2050 formula properties and 84 logic blocks, prioritizing the 43 substantive behavior blocks.
...
| Input | Value |
|---|---|
| Source YAML | ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/Project Planning Screen.pa.yaml |
| Tables | Aquisition Statuses; Aquisition Types; Custom Choice Values; Permits; Project Plannings; Projects |
...
| Feature path | src/features/pm/planning |
| Rule prefix | PLAN |

## Build Outputs

- Screen component, model hook, rules module, and tests under src/features/pm/planning.
- Route registration for /projects/:projectId/planning.
...
- Test evidence for each PLAN-FX-* rule.
...
| Controls inventoried | 178 |
| Formula properties classified | 2050 |
| Logic blocks audited | 84 |
| Substantive logic blocks audited | 43 |
| Stable rule IDs in this plan | 12 |
| Dataverse tables mapped | Aquisition Statuses; Aquisition Types; Custom Choice Values; Permits; Project Plannings; Projects |
