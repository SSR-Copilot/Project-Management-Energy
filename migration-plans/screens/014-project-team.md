...
| Canvas screen YAML | ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/Project General Team Screen.pa.yaml |
| Existing target feature path | src/features/pm/team |
| Recommended route | /projects/:projectId/team |
...
| Risk | Low |
| Controls | 66 |
| Formula properties | 946 |
| Logic blocks reviewed | 29 |
...
Project Member Descriptions; Project Members; Projects; Users
...
|---|---|---|---|
| TEAM-FX-001 | Screen bootstrap and loading gates | ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/Project General Team Screen.pa.yaml OnVisible, Visible, DisplayMode, and initial collection formulas. | Screen model initializes route state, query keys, permissions, and draft collections before enabling commands. |
| TEAM-FX-002 | Project/context filtering | Canvas Filter, LookUp, SortByColumns, and Search formulas over: Project Member Descriptions; Project Members; Projects; Users. | Repository query functions accept typed ids/filters and mirror Canvas row inclusion rules. |
| TEAM-FX-003 | Command enablement | Button/icon DisplayMode, command-bar Items, and popup visibility formulas. | rules.ts exposes pure predicates for add/edit/save/delete/submit availability. |
| TEAM-FX-004 | Validation and required-field rules | If, IsBlank, Notify, border/color formulas, and save guards. | Validation returns field errors plus blocking summary before any mutation executes. |
| TEAM-FX-005 | Patch/remove mutation behavior | Patch, Remove, UpdateIf, and ForAll formulas in behavior properties. | Dataverse service batches equivalent changes with optimistic UI only where Canvas semantics allow it. |
| TEAM-FX-006 | Derived numeric/date values | DateAdd, Round, Value, Coalesce, percentage/currency formatting formulas. | Pure calculation helpers preserve Canvas blank/zero/rounding semantics and have table-driven tests. |
| TEAM-FX-007 | Local collection lifecycle | ClearCollect, Collect, UpdateContext, and Set formulas. | Local state models Canvas working collections and is reset on project/screen change. |
| TEAM-FX-008 | Error/info/confirmation panels | Notify, popup component variables, and confirmation component output formulas. | Shared dialog/toast services expose the same blocking/non-blocking decisions. |
| TEAM-FX-009 | Security and role visibility | Role-gated Visible/DisplayMode formulas plus Dataverse role matrix. | Client hides controls for UX, while Dataverse/custom APIs enforce writes. |
| TEAM-FX-010 | Navigation and return behavior | Navigate, Back, Launch, and Param formulas. | React Router routes preserve projectId and replace cross-app launch where applicable. |
| TEAM-FX-011 | PCF/control event mapping | PeoplePicker; PowerCAT CommandBar; Creator Kit Icon. | Replace PCF events with typed React component callbacks; preserve payload shape, selection, sorting, and action pulses. |
| TEAM-FX-012 | Flow/recalculation side effects | None active; Teams/SPO updates are backend/server-triggered. | Flow calls go through flowClient.ts; missing flows remain implementation blockers until resolved. |
...
None active; Teams/SPO updates are backend/server-triggered
...
PeoplePicker; PowerCAT CommandBar; Creator Kit Icon
...
Add rule tests for every TEAM-FX-* row. Add component tests for load/empty/error/read-only/edit/save/delete paths. Add integration tests around each Dataverse mutation and flow wrapper relevant to this screen. High-risk screens also need regression tests for known Canvas defects or intentional divergences.

## 19 Implementation Files

| Kind | Target |
|---|---|
| Screen component | src/features/pm/team/Screen.tsx |
| Rules | src/features/pm/team/rules.ts |
| Rule tests | src/features/pm/team/rules.test.ts |
| Hooks/model | src/features/pm/team/hooks.ts or src/features/pm/team/useScreenModel.ts |
| Repository/service calls | Shared Dataverse services under the target app data layer |
| Route registration | Route config for /projects/:projectId/team |

## 20 Existing Code App Gap Notes

People lookup and role display are the important UI parity items. Treat existing React code as a helpful starting point only after comparing it to the Canvas YAML. Where the focused cost app and unified skeleton disagree, prefer Canvas behavior plus the global architecture plan.
...
Risk level: Low. Dependencies: General Data. Completion status: COMPLETE. Any missing flow, unverified role privilege, or PCF behavior gap must stay visible in the acceptance checklist until closed with source evidence.
...
|---|---|---|
| ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/Project General Team Screen.pa.yaml | src/features/pm/team/Screen.tsx | Visual/workflow parity review |
| Canvas formulas counted: 946 | src/features/pm/team/rules.ts and hooks | TEAM-FX-* tests |
| Logic blocks counted: 29 | Service/model/UI tests | Coverage report below |
| Tables: Project Member Descriptions; Project Members; Projects; Users | Dataverse repositories | Query/mutation tests |
| PCF: PeoplePicker; PowerCAT CommandBar; Creator Kit Icon | Fluent/React replacement components | Interaction tests |
| Flow: None active; Teams/SPO updates are backend/server-triggered | flowClient.ts wrappers/custom APIs | Payload/error tests |
...
- Inspect ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/Project General Team Screen.pa.yaml directly before coding.
- Confirm target path src/features/pm/team exists or create it using existing repo conventions.
- Implement TEAM-FX-* rule tests before wiring UI mutations.
- Preserve route /projects/:projectId/team and dependency notes.
...
Implement **Project General Team Screen** as a production Code App screen at /projects/:projectId/team using source ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/Project General Team Screen.pa.yaml. Preserve all behavior represented by 946 formula properties and 29 logic blocks, prioritizing the 16 substantive behavior blocks.
...
| Input | Value |
|---|---|
| Source YAML | ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/Project General Team Screen.pa.yaml |
| Tables | Project Member Descriptions; Project Members; Projects; Users |
| PCF/control dependencies | PeoplePicker; PowerCAT CommandBar; Creator Kit Icon |
| Flow/backend dependencies | None active; Teams/SPO updates are backend/server-triggered |
| Route | /projects/:projectId/team |
| Feature path | src/features/pm/team |
| Rule prefix | TEAM |

## Build Outputs

- Screen component, model hook, rules module, and tests under src/features/pm/team.
- Route registration for /projects/:projectId/team.
...
- Test evidence for each TEAM-FX-* rule.
...
| Controls inventoried | 66 |
| Formula properties classified | 946 |
| Logic blocks audited | 29 |
| Substantive logic blocks audited | 16 |
| Stable rule IDs in this plan | 12 |
| Dataverse tables mapped | Project Member Descriptions; Project Members; Projects; Users |
| PCF/control dependencies mapped | PeoplePicker; PowerCAT CommandBar; Creator Kit Icon |
| Flow dependencies mapped | None active; Teams/SPO updates are backend/server-triggered |
