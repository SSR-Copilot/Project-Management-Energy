# Capex Costs Screen migration plan

Plan id: 019-capex. Status: **COMPLETE**. This is planning documentation only; it does not implement the migration.

## 1 Purpose

Maintain DEVEX/CAPEX project costs contract/country filters add-standard behavior and summary grids. The target is one React/TypeScript Power Apps Code App route, not a re-hosted Canvas screen. The screen must preserve Canvas behavior while replacing Canvas globals, controls, and PCFs with explicit React state, Dataverse services, Fluent UI, and tested rule modules.

## 2 Source Evidence

| Evidence | Path or value |
|---|---|
| Canvas app | Project Costs |
| Canvas screen YAML | ProjectCosts-CodeApp/_extracted/msapp/projectcosts/Src/Capex Costs Screen.pa.yaml |
| Existing target feature path | src/features/cost/capex-costs |
| Recommended route | /costs/:projectId/capex |
...
BoP Projects Contracts; CAPEX Account Lists; CAPEX Costs; CAPEX Project Contracts; CAPEX Standard Assumptions; Countries; Devex/Capex Standard Assumptions; Generators; Project Generator Clusters; Projects; Technologies
...
|---|---|---|---|
| CAPEX-FX-001 | Screen bootstrap and loading gates | ProjectCosts-CodeApp/_extracted/msapp/projectcosts/Src/Capex Costs Screen.pa.yaml OnVisible, Visible, DisplayMode, and initial collection formulas. | Screen model initializes route state, query keys, permissions, and draft collections before enabling commands. |
| CAPEX-FX-002 | Project/context filtering | Canvas Filter, LookUp, SortByColumns, and Search formulas over: BoP Projects Contracts; CAPEX Account Lists; CAPEX Costs; CAPEX Project Contracts; CAPEX Standard Assumptions; Countries; Devex/Capex Standard Assumptions; Generators; Project Generator Clusters; Projects; Technologies. | Repository query functions accept typed ids/filters and mirror Canvas row inclusion rules. |
| CAPEX-FX-003 | Command enablement | Button/icon DisplayMode, command-bar Items, and popup visibility formulas. | rules.ts exposes pure predicates for add/edit/save/delete/submit availability. |
| CAPEX-FX-004 | Validation and required-field rules | If, IsBlank, Notify, border/color formulas, and save guards. | Validation returns field errors plus blocking summary before any mutation executes. |
| CAPEX-FX-005 | Patch/remove mutation behavior | Patch, Remove, UpdateIf, and ForAll formulas in behavior properties. | Dataverse service batches equivalent changes with optimistic UI only where Canvas semantics allow it. |
| CAPEX-FX-006 | Derived numeric/date values | DateAdd, Round, Value, Coalesce, percentage/currency formatting formulas. | Pure calculation helpers preserve Canvas blank/zero/rounding semantics and have table-driven tests. |
| CAPEX-FX-007 | Local collection lifecycle | ClearCollect, Collect, UpdateContext, and Set formulas. | Local state models Canvas working collections and is reset on project/screen change. |
| CAPEX-FX-008 | Error/info/confirmation panels | Notify, popup component variables, and confirmation component output formulas. | Shared dialog/toast services expose the same blocking/non-blocking decisions. |
| CAPEX-FX-009 | Security and role visibility | Role-gated Visible/DisplayMode formulas plus Dataverse role matrix. | Client hides controls for UX, while Dataverse/custom APIs enforce writes. |
| CAPEX-FX-010 | Navigation and return behavior | Navigate, Back, Launch, and Param formulas. | React Router routes preserve projectId and replace cross-app launch where applicable. |
| CAPEX-FX-011 | PCF/control event mapping | vsb_Dev.DevexCapexSummaryPCF plus PowerCAT controls. | Replace PCF events with typed React component callbacks; preserve payload shape, selection, sorting, and action pulses. |
| CAPEX-FX-012 | Flow/recalculation side effects | LoadDevexCapexCostTotalCapacityTrigger when recalculation is required. | Flow calls go through flowClient.ts; missing flows remain implementation blockers until resolved. |
...
LoadDevexCapexCostTotalCapacityTrigger when recalculation is required
...
vsb_Dev.DevexCapexSummaryPCF plus PowerCAT controls
...
Add rule tests for every CAPEX-FX-* row. Add component tests for load/empty/error/read-only/edit/save/delete paths. Add integration tests around each Dataverse mutation and flow wrapper relevant to this screen. High-risk screens also need regression tests for known Canvas defects or intentional divergences.

## 19 Implementation Files

| Kind | Target |
|---|---|
| Screen component | src/features/cost/capex-costs/Screen.tsx |
| Rules | src/features/cost/capex-costs/rules.ts |
| Rule tests | src/features/cost/capex-costs/rules.test.ts |
| Hooks/model | src/features/cost/capex-costs/hooks.ts or src/features/cost/capex-costs/useScreenModel.ts |
| Repository/service calls | Shared Dataverse services under the target app data layer |
| Route registration | Route config for /costs/:projectId/capex |

## 20 Existing Code App Gap Notes

Focused app currently simplifies AddStandard; restore standard-contract options and Canvas add behavior. Treat existing React code as a helpful starting point only after comparing it to the Canvas YAML. Where the focused cost app and unified skeleton disagree, prefer Canvas behavior plus the global architecture plan.
...
Risk level: High. Dependencies: Admin CAPEX Accounts; Admin Cost; Admin Milestones. Completion status: COMPLETE. Any missing flow, unverified role privilege, or PCF behavior gap must stay visible in the acceptance checklist until closed with source evidence.
...
|---|---|---|
| ProjectCosts-CodeApp/_extracted/msapp/projectcosts/Src/Capex Costs Screen.pa.yaml | src/features/cost/capex-costs/Screen.tsx | Visual/workflow parity review |
| Canvas formulas counted: 5027 | src/features/cost/capex-costs/rules.ts and hooks | CAPEX-FX-* tests |
| Logic blocks counted: 294 | Service/model/UI tests | Coverage report below |
| Tables: BoP Projects Contracts; CAPEX Account Lists; CAPEX Costs; CAPEX Project Contracts; CAPEX Standard Assumptions; Countries; Devex/Capex Standard Assumptions; Generators; Project Generator Clusters; Projects; Technologies | Dataverse repositories | Query/mutation tests |
| PCF: vsb_Dev.DevexCapexSummaryPCF plus PowerCAT controls | Fluent/React replacement components | Interaction tests |
| Flow: LoadDevexCapexCostTotalCapacityTrigger when recalculation is required | flowClient.ts wrappers/custom APIs | Payload/error tests |
...
- Inspect ProjectCosts-CodeApp/_extracted/msapp/projectcosts/Src/Capex Costs Screen.pa.yaml directly before coding.
- Confirm target path src/features/cost/capex-costs exists or create it using existing repo conventions.
- Implement CAPEX-FX-* rule tests before wiring UI mutations.
- Preserve route /costs/:projectId/capex and dependency notes.
...
Implement **Capex Costs Screen** as a production Code App screen at /costs/:projectId/capex using source ProjectCosts-CodeApp/_extracted/msapp/projectcosts/Src/Capex Costs Screen.pa.yaml. Preserve all behavior represented by 5027 formula properties and 294 logic blocks, prioritizing the 180 substantive behavior blocks.
...
| Input | Value |
|---|---|
| Source YAML | ProjectCosts-CodeApp/_extracted/msapp/projectcosts/Src/Capex Costs Screen.pa.yaml |
| Tables | BoP Projects Contracts; CAPEX Account Lists; CAPEX Costs; CAPEX Project Contracts; CAPEX Standard Assumptions; Countries; Devex/Capex Standard Assumptions; Generators; Project Generator Clusters; Projects; Technologies |
| PCF/control dependencies | vsb_Dev.DevexCapexSummaryPCF plus PowerCAT controls |
| Flow/backend dependencies | LoadDevexCapexCostTotalCapacityTrigger when recalculation is required |
| Route | /costs/:projectId/capex |
| Feature path | src/features/cost/capex-costs |
| Rule prefix | CAPEX |

## Build Outputs

- Screen component, model hook, rules module, and tests under src/features/cost/capex-costs.
- Route registration for /costs/:projectId/capex.
...
- Test evidence for each CAPEX-FX-* rule.
...
| Controls inventoried | 355 |
| Formula properties classified | 5027 |
| Logic blocks audited | 294 |
| Substantive logic blocks audited | 180 |
| Stable rule IDs in this plan | 12 |
| Dataverse tables mapped | BoP Projects Contracts; CAPEX Account Lists; CAPEX Costs; CAPEX Project Contracts; CAPEX Standard Assumptions; Countries; Devex/Capex Standard Assumptions; Generators; Project Generator Clusters; Projects; Technologies |
| PCF/control dependencies mapped | vsb_Dev.DevexCapexSummaryPCF plus PowerCAT controls |
| Flow dependencies mapped | LoadDevexCapexCostTotalCapacityTrigger when recalculation is required |
