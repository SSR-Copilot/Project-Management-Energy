# Opex Costs Screen migration plan

Plan id: 021-opex. Status: **COMPLETE**. This is planning documentation only; it does not implement the migration.

## 1 Purpose

Maintain OPEX cost assumptions and project OPEX rows by period generator and subaccount. The target is one React/TypeScript Power Apps Code App route, not a re-hosted Canvas screen. The screen must preserve Canvas behavior while replacing Canvas globals, controls, and PCFs with explicit React state, Dataverse services, Fluent UI, and tested rule modules.
...
| Canvas screen YAML | ProjectCosts-CodeApp/_extracted/msapp/projectcosts/Src/Opex Costs Screen.pa.yaml |
| Existing target feature path | src/features/cost/opex-costs |
| Recommended route | /costs/:projectId/opex |
...
Countries; Generators; Opex Project Costs; Opex Standard Assumptions; Opex Subaccounts; Project Generator Clusters; Projects
...
|---|---|---|---|
| OPEX-FX-001 | Screen bootstrap and loading gates | ProjectCosts-CodeApp/_extracted/msapp/projectcosts/Src/Opex Costs Screen.pa.yaml OnVisible, Visible, DisplayMode, and initial collection formulas. | Screen model initializes route state, query keys, permissions, and draft collections before enabling commands. |
| OPEX-FX-002 | Project/context filtering | Canvas Filter, LookUp, SortByColumns, and Search formulas over: Countries; Generators; Opex Project Costs; Opex Standard Assumptions; Opex Subaccounts; Project Generator Clusters; Projects. | Repository query functions accept typed ids/filters and mirror Canvas row inclusion rules. |
| OPEX-FX-003 | Command enablement | Button/icon DisplayMode, command-bar Items, and popup visibility formulas. | rules.ts exposes pure predicates for add/edit/save/delete/submit availability. |
| OPEX-FX-004 | Validation and required-field rules | If, IsBlank, Notify, border/color formulas, and save guards. | Validation returns field errors plus blocking summary before any mutation executes. |
| OPEX-FX-005 | Patch/remove mutation behavior | Patch, Remove, UpdateIf, and ForAll formulas in behavior properties. | Dataverse service batches equivalent changes with optimistic UI only where Canvas semantics allow it. |
| OPEX-FX-006 | Derived numeric/date values | DateAdd, Round, Value, Coalesce, percentage/currency formatting formulas. | Pure calculation helpers preserve Canvas blank/zero/rounding semantics and have table-driven tests. |
| OPEX-FX-007 | Local collection lifecycle | ClearCollect, Collect, UpdateContext, and Set formulas. | Local state models Canvas working collections and is reset on project/screen change. |
| OPEX-FX-008 | Error/info/confirmation panels | Notify, popup component variables, and confirmation component output formulas. | Shared dialog/toast services expose the same blocking/non-blocking decisions. |
| OPEX-FX-009 | Security and role visibility | Role-gated Visible/DisplayMode formulas plus Dataverse role matrix. | Client hides controls for UX, while Dataverse/custom APIs enforce writes. |
| OPEX-FX-010 | Navigation and return behavior | Navigate, Back, Launch, and Param formulas. | React Router routes preserve projectId and replace cross-app launch where applicable. |
| OPEX-FX-011 | PCF/control event mapping | PowerCAT CommandBar; FluentDetailsList; Creator Kit Icon. | Replace PCF events with typed React component callbacks; preserve payload shape, selection, sorting, and action pulses. |
| OPEX-FX-012 | Flow/recalculation side effects | LoadDevexCapexCostTotalCapacityTrigger when recalculation is required. | Flow calls go through flowClient.ts; missing flows remain implementation blockers until resolved. |
...
Add rule tests for every OPEX-FX-* row. Add component tests for load/empty/error/read-only/edit/save/delete paths. Add integration tests around each Dataverse mutation and flow wrapper relevant to this screen. High-risk screens also need regression tests for known Canvas defects or intentional divergences.

## 19 Implementation Files

| Kind | Target |
|---|---|
| Screen component | src/features/cost/opex-costs/Screen.tsx |
| Rules | src/features/cost/opex-costs/rules.ts |
| Rule tests | src/features/cost/opex-costs/rules.test.ts |
| Hooks/model | src/features/cost/opex-costs/hooks.ts or src/features/cost/opex-costs/useScreenModel.ts |
| Repository/service calls | Shared Dataverse services under the target app data layer |
| Route registration | Route config for /costs/:projectId/opex |

## 20 Existing Code App Gap Notes

No screen recording was found; layout parity is inferred from Canvas control tree and cost-app conventions. Treat existing React code as a helpful starting point only after comparing it to the Canvas YAML. Where the focused cost app and unified skeleton disagree, prefer Canvas behavior plus the global architecture plan.
...
|---|---|---|
| ProjectCosts-CodeApp/_extracted/msapp/projectcosts/Src/Opex Costs Screen.pa.yaml | src/features/cost/opex-costs/Screen.tsx | Visual/workflow parity review |
| Canvas formulas counted: 4268 | src/features/cost/opex-costs/rules.ts and hooks | OPEX-FX-* tests |
| Logic blocks counted: 163 | Service/model/UI tests | Coverage report below |
| Tables: Countries; Generators; Opex Project Costs; Opex Standard Assumptions; Opex Subaccounts; Project Generator Clusters; Projects | Dataverse repositories | Query/mutation tests |
...
- Inspect ProjectCosts-CodeApp/_extracted/msapp/projectcosts/Src/Opex Costs Screen.pa.yaml directly before coding.
- Confirm target path src/features/cost/opex-costs exists or create it using existing repo conventions.
- Implement OPEX-FX-* rule tests before wiring UI mutations.
- Preserve route /costs/:projectId/opex and dependency notes.
...
Implement **Opex Costs Screen** as a production Code App screen at /costs/:projectId/opex using source ProjectCosts-CodeApp/_extracted/msapp/projectcosts/Src/Opex Costs Screen.pa.yaml. Preserve all behavior represented by 4268 formula properties and 163 logic blocks, prioritizing the 65 substantive behavior blocks.
...
| Input | Value |
|---|---|
| Source YAML | ProjectCosts-CodeApp/_extracted/msapp/projectcosts/Src/Opex Costs Screen.pa.yaml |
| Tables | Countries; Generators; Opex Project Costs; Opex Standard Assumptions; Opex Subaccounts; Project Generator Clusters; Projects |
...
| Route | /costs/:projectId/opex |
| Feature path | src/features/cost/opex-costs |
| Rule prefix | OPEX |

## Build Outputs

- Screen component, model hook, rules module, and tests under src/features/cost/opex-costs.
- Route registration for /costs/:projectId/opex.
...
- Test evidence for each OPEX-FX-* rule.
...
| Controls inventoried | 230 |
| Formula properties classified | 4268 |
| Logic blocks audited | 163 |
| Substantive logic blocks audited | 65 |
| Stable rule IDs in this plan | 12 |
| Dataverse tables mapped | Countries; Generators; Opex Project Costs; Opex Standard Assumptions; Opex Subaccounts; Project Generator Clusters; Projects |
