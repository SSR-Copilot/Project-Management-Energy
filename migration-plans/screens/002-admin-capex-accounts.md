...
| Canvas screen YAML | ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/Admin CAPEX Accounts.pa.yaml |
| Existing target feature path | src/features/admin/capex-accounts |
| Recommended route | /admin/capex-accounts |
...
CAPEX Account Lists; CAPEX Costs; CAPEX Project Contracts
...
|---|---|---|---|
| ADCAPEX-FX-001 | Screen bootstrap and loading gates | ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/Admin CAPEX Accounts.pa.yaml OnVisible, Visible, DisplayMode, and initial collection formulas. | Screen model initializes route state, query keys, permissions, and draft collections before enabling commands. |
| ADCAPEX-FX-002 | Project/context filtering | Canvas Filter, LookUp, SortByColumns, and Search formulas over: CAPEX Account Lists; CAPEX Costs; CAPEX Project Contracts. | Repository query functions accept typed ids/filters and mirror Canvas row inclusion rules. |
| ADCAPEX-FX-003 | Command enablement | Button/icon DisplayMode, command-bar Items, and popup visibility formulas. | rules.ts exposes pure predicates for add/edit/save/delete/submit availability. |
| ADCAPEX-FX-004 | Validation and required-field rules | If, IsBlank, Notify, border/color formulas, and save guards. | Validation returns field errors plus blocking summary before any mutation executes. |
| ADCAPEX-FX-005 | Patch/remove mutation behavior | Patch, Remove, UpdateIf, and ForAll formulas in behavior properties. | Dataverse service batches equivalent changes with optimistic UI only where Canvas semantics allow it. |
| ADCAPEX-FX-006 | Derived numeric/date values | DateAdd, Round, Value, Coalesce, percentage/currency formatting formulas. | Pure calculation helpers preserve Canvas blank/zero/rounding semantics and have table-driven tests. |
| ADCAPEX-FX-007 | Local collection lifecycle | ClearCollect, Collect, UpdateContext, and Set formulas. | Local state models Canvas working collections and is reset on project/screen change. |
| ADCAPEX-FX-008 | Error/info/confirmation panels | Notify, popup component variables, and confirmation component output formulas. | Shared dialog/toast services expose the same blocking/non-blocking decisions. |
| ADCAPEX-FX-009 | Security and role visibility | Role-gated Visible/DisplayMode formulas plus Dataverse role matrix. | Client hides controls for UX, while Dataverse/custom APIs enforce writes. |
| ADCAPEX-FX-010 | Navigation and return behavior | Navigate, Back, Launch, and Param formulas. | React Router routes preserve projectId and replace cross-app launch where applicable. |
| ADCAPEX-FX-011 | PCF/control event mapping | PowerCAT CommandBar; Creator Kit Icon. | Replace PCF events with typed React component callbacks; preserve payload shape, selection, sorting, and action pulses. |
| ADCAPEX-FX-012 | Flow/recalculation side effects | None. | Flow calls go through flowClient.ts; missing flows remain implementation blockers until resolved. |
...
Add rule tests for every ADCAPEX-FX-* row. Add component tests for load/empty/error/read-only/edit/save/delete paths. Add integration tests around each Dataverse mutation and flow wrapper relevant to this screen. High-risk screens also need regression tests for known Canvas defects or intentional divergences.

## 19 Implementation Files

| Kind | Target |
|---|---|
| Screen component | src/features/admin/capex-accounts/Screen.tsx |
| Rules | src/features/admin/capex-accounts/rules.ts |
| Rule tests | src/features/admin/capex-accounts/rules.test.ts |
| Hooks/model | src/features/admin/capex-accounts/hooks.ts or src/features/admin/capex-accounts/useScreenModel.ts |
| Repository/service calls | Shared Dataverse services under the target app data layer |
| Route registration | Route config for /admin/capex-accounts |

## 20 Existing Code App Gap Notes

Early master-data prerequisite for Admin Cost Admin Contract and Capex Costs. Treat existing React code as a helpful starting point only after comparing it to the Canvas YAML. Where the focused cost app and unified skeleton disagree, prefer Canvas behavior plus the global architecture plan.
...
Risk level: Medium. Dependencies: None. Completion status: COMPLETE. Any missing flow, unverified role privilege, or PCF behavior gap must stay visible in the acceptance checklist until closed with source evidence.
...
|---|---|---|
| ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/Admin CAPEX Accounts.pa.yaml | src/features/admin/capex-accounts/Screen.tsx | Visual/workflow parity review |
| Canvas formulas counted: 1234 | src/features/admin/capex-accounts/rules.ts and hooks | ADCAPEX-FX-* tests |
| Logic blocks counted: 68 | Service/model/UI tests | Coverage report below |
| Tables: CAPEX Account Lists; CAPEX Costs; CAPEX Project Contracts | Dataverse repositories | Query/mutation tests |
...
- Inspect ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/Admin CAPEX Accounts.pa.yaml directly before coding.
- Confirm target path src/features/admin/capex-accounts exists or create it using existing repo conventions.
- Implement ADCAPEX-FX-* rule tests before wiring UI mutations.
- Preserve route /admin/capex-accounts and dependency notes.
...
Implement **Admin CAPEX Accounts** as a production Code App screen at /admin/capex-accounts using source ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/Admin CAPEX Accounts.pa.yaml. Preserve all behavior represented by 1234 formula properties and 68 logic blocks, prioritizing the 26 substantive behavior blocks.
...
| Input | Value |
|---|---|
| Source YAML | ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/Admin CAPEX Accounts.pa.yaml |
| Tables | CAPEX Account Lists; CAPEX Costs; CAPEX Project Contracts |
| PCF/control dependencies | PowerCAT CommandBar; Creator Kit Icon |
| Flow/backend dependencies | None |
| Route | /admin/capex-accounts |
| Feature path | src/features/admin/capex-accounts |
| Rule prefix | ADCAPEX |

## Build Outputs

- Screen component, model hook, rules module, and tests under src/features/admin/capex-accounts.
- Route registration for /admin/capex-accounts.
...
- Test evidence for each ADCAPEX-FX-* rule.
...
| Controls inventoried | 90 |
| Formula properties classified | 1234 |
| Logic blocks audited | 68 |
| Substantive logic blocks audited | 26 |
| Stable rule IDs in this plan | 12 |
| Dataverse tables mapped | CAPEX Account Lists; CAPEX Costs; CAPEX Project Contracts |
