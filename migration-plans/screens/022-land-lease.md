# Land Lease Costs Screen migration plan

Plan id: 022-lease. Status: **COMPLETE**. This is planning documentation only; it does not implement the migration.

## 1 Purpose

Maintain land lease project costs standard assumptions subaccounts and generator/country relationships. The target is one React/TypeScript Power Apps Code App route, not a re-hosted Canvas screen. The screen must preserve Canvas behavior while replacing Canvas globals, controls, and PCFs with explicit React state, Dataverse services, Fluent UI, and tested rule modules.
...
| Canvas screen YAML | ProjectCosts-CodeApp/_extracted/msapp/projectcosts/Src/Land Lease Costs Screen.pa.yaml |
| Existing target feature path | src/features/cost/land-lease |
| Recommended route | /costs/:projectId/land-lease |
...
Countries; Generators; Land Lease Project Costs; Land Lease Standard Assumptions; Land Lease Subaccounts; Project Generator Clusters; Projects; Technologies; CAPEX Account Lists; Country Inflation Profiles
...
|---|---|---|---|
| LEASE-FX-001 | Screen bootstrap and loading gates | ProjectCosts-CodeApp/_extracted/msapp/projectcosts/Src/Land Lease Costs Screen.pa.yaml OnVisible, Visible, DisplayMode, and initial collection formulas. | Screen model initializes route state, query keys, permissions, and draft collections before enabling commands. |
| LEASE-FX-002 | Project/context filtering | Canvas Filter, LookUp, SortByColumns, and Search formulas over: Countries; Generators; Land Lease Project Costs; Land Lease Standard Assumptions; Land Lease Subaccounts; Project Generator Clusters; Projects; Technologies; CAPEX Account Lists; Country Inflation Profiles. | Repository query functions accept typed ids/filters and mirror Canvas row inclusion rules. |
| LEASE-FX-003 | Command enablement | Button/icon DisplayMode, command-bar Items, and popup visibility formulas. | rules.ts exposes pure predicates for add/edit/save/delete/submit availability. |
| LEASE-FX-004 | Validation and required-field rules | If, IsBlank, Notify, border/color formulas, and save guards. | Validation returns field errors plus blocking summary before any mutation executes. |
| LEASE-FX-005 | Patch/remove mutation behavior | Patch, Remove, UpdateIf, and ForAll formulas in behavior properties. | Dataverse service batches equivalent changes with optimistic UI only where Canvas semantics allow it. |
| LEASE-FX-006 | Derived numeric/date values | DateAdd, Round, Value, Coalesce, percentage/currency formatting formulas. | Pure calculation helpers preserve Canvas blank/zero/rounding semantics and have table-driven tests. |
| LEASE-FX-007 | Local collection lifecycle | ClearCollect, Collect, UpdateContext, and Set formulas. | Local state models Canvas working collections and is reset on project/screen change. |
| LEASE-FX-008 | Error/info/confirmation panels | Notify, popup component variables, and confirmation component output formulas. | Shared dialog/toast services expose the same blocking/non-blocking decisions. |
| LEASE-FX-009 | Security and role visibility | Role-gated Visible/DisplayMode formulas plus Dataverse role matrix. | Client hides controls for UX, while Dataverse/custom APIs enforce writes. |
| LEASE-FX-010 | Navigation and return behavior | Navigate, Back, Launch, and Param formulas. | React Router routes preserve projectId and replace cross-app launch where applicable. |
| LEASE-FX-011 | PCF/control event mapping | PowerCAT CommandBar; FluentDetailsList; Creator Kit Icon. | Replace PCF events with typed React component callbacks; preserve payload shape, selection, sorting, and action pulses. |
| LEASE-FX-012 | Flow/recalculation side effects | LoadDevexCapexCostTotalCapacityTrigger when recalculation is required. | Flow calls go through flowClient.ts; missing flows remain implementation blockers until resolved. |
...
Add rule tests for every LEASE-FX-* row. Add component tests for load/empty/error/read-only/edit/save/delete paths. Add integration tests around each Dataverse mutation and flow wrapper relevant to this screen. High-risk screens also need regression tests for known Canvas defects or intentional divergences.

## 19 Implementation Files

| Kind | Target |
|---|---|
| Screen component | src/features/cost/land-lease/Screen.tsx |
| Rules | src/features/cost/land-lease/rules.ts |
| Rule tests | src/features/cost/land-lease/rules.test.ts |
| Hooks/model | src/features/cost/land-lease/hooks.ts or src/features/cost/land-lease/useScreenModel.ts |
| Repository/service calls | Shared Dataverse services under the target app data layer |
| Route registration | Route config for /costs/:projectId/land-lease |

## 20 Existing Code App Gap Notes

No screen recording was found; exact layout is derived from control hierarchy and shared cost UI. Treat existing React code as a helpful starting point only after comparing it to the Canvas YAML. Where the focused cost app and unified skeleton disagree, prefer Canvas behavior plus the global architecture plan.
...
Risk level: Medium. Dependencies: Capex Costs; Admin Cost. Completion status: COMPLETE. Any missing flow, unverified role privilege, or PCF behavior gap must stay visible in the acceptance checklist until closed with source evidence.
...
|---|---|---|
| ProjectCosts-CodeApp/_extracted/msapp/projectcosts/Src/Land Lease Costs Screen.pa.yaml | src/features/cost/land-lease/Screen.tsx | Visual/workflow parity review |
| Canvas formulas counted: 3653 | src/features/cost/land-lease/rules.ts and hooks | LEASE-FX-* tests |
| Logic blocks counted: 156 | Service/model/UI tests | Coverage report below |
| Tables: Countries; Generators; Land Lease Project Costs; Land Lease Standard Assumptions; Land Lease Subaccounts; Project Generator Clusters; Projects; Technologies; CAPEX Account Lists; Country Inflation Profiles | Dataverse repositories | Query/mutation tests |
...
- Inspect ProjectCosts-CodeApp/_extracted/msapp/projectcosts/Src/Land Lease Costs Screen.pa.yaml directly before coding.
- Confirm target path src/features/cost/land-lease exists or create it using existing repo conventions.
- Implement LEASE-FX-* rule tests before wiring UI mutations.
- Preserve route /costs/:projectId/land-lease and dependency notes.
...
Implement **Land Lease Costs Screen** as a production Code App screen at /costs/:projectId/land-lease using source ProjectCosts-CodeApp/_extracted/msapp/projectcosts/Src/Land Lease Costs Screen.pa.yaml. Preserve all behavior represented by 3653 formula properties and 156 logic blocks, prioritizing the 61 substantive behavior blocks.
...
| Input | Value |
|---|---|
| Source YAML | ProjectCosts-CodeApp/_extracted/msapp/projectcosts/Src/Land Lease Costs Screen.pa.yaml |
| Tables | Countries; Generators; Land Lease Project Costs; Land Lease Standard Assumptions; Land Lease Subaccounts; Project Generator Clusters; Projects; Technologies; CAPEX Account Lists; Country Inflation Profiles |
...
| Route | /costs/:projectId/land-lease |
| Feature path | src/features/cost/land-lease |
| Rule prefix | LEASE |

## Build Outputs

- Screen component, model hook, rules module, and tests under src/features/cost/land-lease.
- Route registration for /costs/:projectId/land-lease.
...
- Test evidence for each LEASE-FX-* rule.
...
| Controls inventoried | 201 |
| Formula properties classified | 3653 |
| Logic blocks audited | 156 |
| Substantive logic blocks audited | 61 |
| Stable rule IDs in this plan | 12 |
| Dataverse tables mapped | Countries; Generators; Land Lease Project Costs; Land Lease Standard Assumptions; Land Lease Subaccounts; Project Generator Clusters; Projects; Technologies; CAPEX Account Lists; Country Inflation Profiles |
