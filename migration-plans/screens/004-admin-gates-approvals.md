# Admin Project Gates Approvals Screen migration plan

Plan id: 004-adgate. Status: **COMPLETE**. This is planning documentation only; it does not implement the migration.

## 1 Purpose

Maintain default gate approvals and checklist/country/technology mappings that drive project governance. The target is one React/TypeScript Power Apps Code App route, not a re-hosted Canvas screen. The screen must preserve Canvas behavior while replacing Canvas globals, controls, and PCFs with explicit React state, Dataverse services, Fluent UI, and tested rule modules.
...
| Canvas screen YAML | ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/Admin Project Gates Approvals Screen.pa.yaml |
| Existing target feature path | src/features/admin/gates-approvals |
| Recommended route | /admin/gates-approvals |
...
Check List Default Approvals; Checklist Country And Technologies; Countries; Project Default Approvals; Project Default Checklists; Project States; Projects; Users
...
|---|---|---|---|
| ADGATE-FX-001 | Screen bootstrap and loading gates | ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/Admin Project Gates Approvals Screen.pa.yaml OnVisible, Visible, DisplayMode, and initial collection formulas. | Screen model initializes route state, query keys, permissions, and draft collections before enabling commands. |
| ADGATE-FX-002 | Project/context filtering | Canvas Filter, LookUp, SortByColumns, and Search formulas over: Check List Default Approvals; Checklist Country And Technologies; Countries; Project Default Approvals; Project Default Checklists; Project States; Projects; Users. | Repository query functions accept typed ids/filters and mirror Canvas row inclusion rules. |
| ADGATE-FX-003 | Command enablement | Button/icon DisplayMode, command-bar Items, and popup visibility formulas. | rules.ts exposes pure predicates for add/edit/save/delete/submit availability. |
| ADGATE-FX-004 | Validation and required-field rules | If, IsBlank, Notify, border/color formulas, and save guards. | Validation returns field errors plus blocking summary before any mutation executes. |
| ADGATE-FX-005 | Patch/remove mutation behavior | Patch, Remove, UpdateIf, and ForAll formulas in behavior properties. | Dataverse service batches equivalent changes with optimistic UI only where Canvas semantics allow it. |
| ADGATE-FX-006 | Derived numeric/date values | DateAdd, Round, Value, Coalesce, percentage/currency formatting formulas. | Pure calculation helpers preserve Canvas blank/zero/rounding semantics and have table-driven tests. |
| ADGATE-FX-007 | Local collection lifecycle | ClearCollect, Collect, UpdateContext, and Set formulas. | Local state models Canvas working collections and is reset on project/screen change. |
| ADGATE-FX-008 | Error/info/confirmation panels | Notify, popup component variables, and confirmation component output formulas. | Shared dialog/toast services expose the same blocking/non-blocking decisions. |
| ADGATE-FX-009 | Security and role visibility | Role-gated Visible/DisplayMode formulas plus Dataverse role matrix. | Client hides controls for UX, while Dataverse/custom APIs enforce writes. |
| ADGATE-FX-010 | Navigation and return behavior | Navigate, Back, Launch, and Param formulas. | React Router routes preserve projectId and replace cross-app launch where applicable. |
| ADGATE-FX-011 | PCF/control event mapping | PowerCAT CommandBar; Creator Kit Icon; PeoplePicker. | Replace PCF events with typed React component callbacks; preserve payload shape, selection, sorting, and action pulses. |
| ADGATE-FX-012 | Flow/recalculation side effects | RequestApproverApprovals. | Flow calls go through flowClient.ts; missing flows remain implementation blockers until resolved. |
...
PowerCAT CommandBar; Creator Kit Icon; PeoplePicker
...
Add rule tests for every ADGATE-FX-* row. Add component tests for load/empty/error/read-only/edit/save/delete paths. Add integration tests around each Dataverse mutation and flow wrapper relevant to this screen. High-risk screens also need regression tests for known Canvas defects or intentional divergences.

## 19 Implementation Files

| Kind | Target |
|---|---|
| Screen component | src/features/admin/gates-approvals/Screen.tsx |
| Rules | src/features/admin/gates-approvals/rules.ts |
| Rule tests | src/features/admin/gates-approvals/rules.test.ts |
| Hooks/model | src/features/admin/gates-approvals/hooks.ts or src/features/admin/gates-approvals/useScreenModel.ts |
| Repository/service calls | Shared Dataverse services under the target app data layer |
| Route registration | Route config for /admin/gates-approvals |

## 20 Existing Code App Gap Notes

Security validation is mandatory because this screen owns approval master data. Treat existing React code as a helpful starting point only after comparing it to the Canvas YAML. Where the focused cost app and unified skeleton disagree, prefer Canvas behavior plus the global architecture plan.
...
Risk level: High. Dependencies: Admin Project Default Checklists is a mutual dependency; build the pair together.. Completion status: COMPLETE. Any missing flow, unverified role privilege, or PCF behavior gap must stay visible in the acceptance checklist until closed with source evidence.
...
|---|---|---|
| ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/Admin Project Gates Approvals Screen.pa.yaml | src/features/admin/gates-approvals/Screen.tsx | Visual/workflow parity review |
| Canvas formulas counted: 1745 | src/features/admin/gates-approvals/rules.ts and hooks | ADGATE-FX-* tests |
| Logic blocks counted: 89 | Service/model/UI tests | Coverage report below |
| Tables: Check List Default Approvals; Checklist Country And Technologies; Countries; Project Default Approvals; Project Default Checklists; Project States; Projects; Users | Dataverse repositories | Query/mutation tests |
| PCF: PowerCAT CommandBar; Creator Kit Icon; PeoplePicker | Fluent/React replacement components | Interaction tests |
| Flow: RequestApproverApprovals | flowClient.ts wrappers/custom APIs | Payload/error tests |
...
- Inspect ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/Admin Project Gates Approvals Screen.pa.yaml directly before coding.
- Confirm target path src/features/admin/gates-approvals exists or create it using existing repo conventions.
- Implement ADGATE-FX-* rule tests before wiring UI mutations.
- Preserve route /admin/gates-approvals and dependency notes.
...
Implement **Admin Project Gates Approvals Screen** as a production Code App screen at /admin/gates-approvals using source ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/Admin Project Gates Approvals Screen.pa.yaml. Preserve all behavior represented by 1745 formula properties and 89 logic blocks, prioritizing the 41 substantive behavior blocks.
...
| Input | Value |
|---|---|
| Source YAML | ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/Admin Project Gates Approvals Screen.pa.yaml |
| Tables | Check List Default Approvals; Checklist Country And Technologies; Countries; Project Default Approvals; Project Default Checklists; Project States; Projects; Users |
| PCF/control dependencies | PowerCAT CommandBar; Creator Kit Icon; PeoplePicker |
| Flow/backend dependencies | RequestApproverApprovals |
| Route | /admin/gates-approvals |
| Feature path | src/features/admin/gates-approvals |
| Rule prefix | ADGATE |

## Build Outputs

- Screen component, model hook, rules module, and tests under src/features/admin/gates-approvals.
- Route registration for /admin/gates-approvals.
...
- Test evidence for each ADGATE-FX-* rule.
...
| Controls inventoried | 114 |
| Formula properties classified | 1745 |
| Logic blocks audited | 89 |
| Substantive logic blocks audited | 41 |
| Stable rule IDs in this plan | 12 |
| Dataverse tables mapped | Check List Default Approvals; Checklist Country And Technologies; Countries; Project Default Approvals; Project Default Checklists; Project States; Projects; Users |
| PCF/control dependencies mapped | PowerCAT CommandBar; Creator Kit Icon; PeoplePicker |
| Flow dependencies mapped | RequestApproverApprovals |
