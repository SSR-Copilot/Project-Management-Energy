# App Loading Screen migration plan

Plan id: 007-load. Status: **COMPLETE**. This is planning documentation only; it does not implement the migration.

## 1 Purpose

Bootstrap user role project country generator and launch-parameter state before routing into PM screens. The target is one React/TypeScript Power Apps Code App route, not a re-hosted Canvas screen. The screen must preserve Canvas behavior while replacing Canvas globals, controls, and PCFs with explicit React state, Dataverse services, Fluent UI, and tested rule modules.
...
| Canvas screen YAML | ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/App Loading Screen.pa.yaml |
| Existing target feature path | src/features/pm/app-loading |
...
Assumptions Local Tax Frances/Germanies; Countries; Generators; Project States; Projects
...
|---|---|---|---|
| LOAD-FX-001 | Screen bootstrap and loading gates | ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/App Loading Screen.pa.yaml OnVisible, Visible, DisplayMode, and initial collection formulas. | Screen model initializes route state, query keys, permissions, and draft collections before enabling commands. |
| LOAD-FX-002 | Project/context filtering | Canvas Filter, LookUp, SortByColumns, and Search formulas over: Assumptions Local Tax Frances/Germanies; Countries; Generators; Project States; Projects. | Repository query functions accept typed ids/filters and mirror Canvas row inclusion rules. |
| LOAD-FX-003 | Command enablement | Button/icon DisplayMode, command-bar Items, and popup visibility formulas. | rules.ts exposes pure predicates for add/edit/save/delete/submit availability. |
| LOAD-FX-004 | Validation and required-field rules | If, IsBlank, Notify, border/color formulas, and save guards. | Validation returns field errors plus blocking summary before any mutation executes. |
| LOAD-FX-005 | Patch/remove mutation behavior | Patch, Remove, UpdateIf, and ForAll formulas in behavior properties. | Dataverse service batches equivalent changes with optimistic UI only where Canvas semantics allow it. |
| LOAD-FX-006 | Derived numeric/date values | DateAdd, Round, Value, Coalesce, percentage/currency formatting formulas. | Pure calculation helpers preserve Canvas blank/zero/rounding semantics and have table-driven tests. |
| LOAD-FX-007 | Local collection lifecycle | ClearCollect, Collect, UpdateContext, and Set formulas. | Local state models Canvas working collections and is reset on project/screen change. |
| LOAD-FX-008 | Error/info/confirmation panels | Notify, popup component variables, and confirmation component output formulas. | Shared dialog/toast services expose the same blocking/non-blocking decisions. |
| LOAD-FX-009 | Security and role visibility | Role-gated Visible/DisplayMode formulas plus Dataverse role matrix. | Client hides controls for UX, while Dataverse/custom APIs enforce writes. |
| LOAD-FX-010 | Navigation and return behavior | Navigate, Back, Launch, and Param formulas. | React Router routes preserve projectId and replace cross-app launch where applicable. |
| LOAD-FX-011 | PCF/control event mapping | None. | Replace PCF events with typed React component callbacks; preserve payload shape, selection, sorting, and action pulses. |
| LOAD-FX-012 | Flow/recalculation side effects | None. | Flow calls go through flowClient.ts; missing flows remain implementation blockers until resolved. |
...
Add rule tests for every LOAD-FX-* row. Add component tests for load/empty/error/read-only/edit/save/delete paths. Add integration tests around each Dataverse mutation and flow wrapper relevant to this screen. High-risk screens also need regression tests for known Canvas defects or intentional divergences.

## 19 Implementation Files

| Kind | Target |
|---|---|
| Screen component | src/features/pm/app-loading/Screen.tsx |
| Rules | src/features/pm/app-loading/rules.ts |
| Rule tests | src/features/pm/app-loading/rules.test.ts |
| Hooks/model | src/features/pm/app-loading/hooks.ts or src/features/pm/app-loading/useScreenModel.ts |
| Repository/service calls | Shared Dataverse services under the target app data layer |
| Route registration | Route config for /loading |

## 20 Existing Code App Gap Notes

Every downstream PM route relies on deterministic replacement of Canvas global initialization. Treat existing React code as a helpful starting point only after comparing it to the Canvas YAML. Where the focused cost app and unified skeleton disagree, prefer Canvas behavior plus the global architecture plan.
...
|---|---|---|
| ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/App Loading Screen.pa.yaml | src/features/pm/app-loading/Screen.tsx | Visual/workflow parity review |
| Canvas formulas counted: 256 | src/features/pm/app-loading/rules.ts and hooks | LOAD-FX-* tests |
| Logic blocks counted: 10 | Service/model/UI tests | Coverage report below |
| Tables: Assumptions Local Tax Frances/Germanies; Countries; Generators; Project States; Projects | Dataverse repositories | Query/mutation tests |
| PCF: None | Fluent/React replacement components | Interaction tests |
...
- Inspect ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/App Loading Screen.pa.yaml directly before coding.
- Confirm target path src/features/pm/app-loading exists or create it using existing repo conventions.
- Implement LOAD-FX-* rule tests before wiring UI mutations.
- Preserve route /loading and dependency notes.
...
Implement **App Loading Screen** as a production Code App screen at /loading using source ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/App Loading Screen.pa.yaml. Preserve all behavior represented by 256 formula properties and 10 logic blocks, prioritizing the 6 substantive behavior blocks.
...
| Input | Value |
|---|---|
| Source YAML | ProjectCosts-CodeApp/_extracted/msapp/projectmanagement/Src/App Loading Screen.pa.yaml |
| Tables | Assumptions Local Tax Frances/Germanies; Countries; Generators; Project States; Projects |
| PCF/control dependencies | None |
| Flow/backend dependencies | None |
| Route | /loading |
| Feature path | src/features/pm/app-loading |
| Rule prefix | LOAD |

## Build Outputs

- Screen component, model hook, rules module, and tests under src/features/pm/app-loading.
- Route registration for /loading.
...
- Test evidence for each LOAD-FX-* rule.
...
| Controls inventoried | 21 |
| Formula properties classified | 256 |
| Logic blocks audited | 10 |
| Substantive logic blocks audited | 6 |
| Stable rule IDs in this plan | 12 |
| Dataverse tables mapped | Assumptions Local Tax Frances/Germanies; Countries; Generators; Project States; Projects |
| PCF/control dependencies mapped | None |
